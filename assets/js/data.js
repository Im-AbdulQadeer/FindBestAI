/* ════════════════════════════════════════
   AI HUB — SHARED DATA
════════════════════════════════════════ */

const CATS = [
  {icon:'🧠',name:'AI Assistants',count:30},
  {icon:'🎨',name:'Image Generation',count:28},
  {icon:'🎬',name:'Video Editing',count:25},
  {icon:'💻',name:'Code Assistant',count:28},
  {icon:'✍️',name:'Writing & Copy',count:28},
  {icon:'🔬',name:'Research',count:20},
  {icon:'🎙️',name:'Voice & Speech',count:20},
  {icon:'🎵',name:'Audio & Music',count:18},
  {icon:'🎭',name:'Graphic Design',count:22},
  {icon:'📄',name:'PDF & Docs',count:20},
  {icon:'📝',name:'Notes & PKM',count:18},
  {icon:'📊',name:'Data Analysis',count:20},
  {icon:'🤖',name:'Chatbots',count:22},
  {icon:'🌐',name:'SEO & Marketing',count:25},
  {icon:'🔗',name:'Automation',count:20},
  {icon:'📸',name:'Photo Editing',count:22},
  {icon:'📧',name:'Email & Outreach',count:18},
  {icon:'📹',name:'Video Avatars',count:15},
  {icon:'⚡',name:'Productivity',count:22},
  {icon:'🧊',name:'3D & AR',count:18},
];

const TOOLS = [
  {
    id: 1,
    name: 'ChatGPT',
    cat: 'AI Assistants',
    emoji: '🤖',
    bg: 'linear-gradient(135deg,#10b981,#059669)',
    pricing: 'freemium',
    badge: 'best',
    url: 'https://chat.openai.com',
    desc: 'The world\'s most popular AI assistant by OpenAI. Write, code, research and create with GPT-5.5, now free for everyone.',
    rating: 4.9,
    reviews: '98.2k',
    tags: [
      'GPT-5.5',
      'writing',
      'coding',
      'reasoning'
    ],
    features: [
      'GPT-5.5 free for all users',
      'Custom GPTs marketplace',
      'Vision & DALL-E image gen',
      'Code interpreter & data analysis',
      'Web browsing & plugins'
    ],
    alts: [
      {n:'Claude',t:'Freemium'},
      {n:'Gemini',t:'Free'},
      {n:'DeepSeek',t:'Free'}
    ],
    price: 'Free / $20/mo',
    freeAlt: 'Gemini'
  },
  {
    id: 2,
    name: 'Claude',
    cat: 'AI Assistants',
    emoji: '🧠',
    bg: 'linear-gradient(135deg,#c084fc,#a855f7)',
    pricing: 'freemium',
    badge: 'best',
    url: 'https://claude.ai',
    desc: 'Anthropic\'s Claude is renowned for nuanced reasoning, long-context understanding up to 200K tokens, and safe, honest responses.',
    rating: 4.9,
    reviews: '22.1k',
    tags: [
      'reasoning',
      '200K context',
      'coding',
      'analysis'
    ],
    features: [
      '200K token context window',
      'Claude 3.5 Sonnet & Opus',
      'Artifacts for code & docs',
      'Vision & file analysis',
      'API access via Anthropic'
    ],
    alts: [
      {n:'ChatGPT',t:'Freemium'},
      {n:'Gemini',t:'Free'},
      {n:'Mistral',t:'Free'}
    ],
    price: 'Free / $20/mo',
    freeAlt: 'ChatGPT'
  },
  {
    id: 3,
    name: 'Gemini',
    cat: 'AI Assistants',
    emoji: '♊',
    bg: 'linear-gradient(135deg,#4285f4,#0f9d58)',
    pricing: 'freemium',
    badge: 'best',
    url: 'https://gemini.google.com',
    desc: 'Google\'s most capable AI model. Deep integration with Google Workspace, real-time search, and multimodal understanding.',
    rating: 4.8,
    reviews: '31.5k',
    tags: [
      'Google',
      'multimodal',
      'search',
      'workspace'
    ],
    features: [
      'Real-time Google Search grounding',
      'Gemini 2.0 Flash & Pro',
      'Google Docs/Sheets/Gmail integration',
      'Image, audio & video understanding',
      '1M token context in Advanced'
    ],
    alts: [
      {n:'ChatGPT',t:'Freemium'},
      {n:'Claude',t:'Freemium'},
      {n:'Copilot',t:'Free'}
    ],
    price: 'Free / $19.99/mo',
    freeAlt: 'ChatGPT'
  },
  {
    id: 4,
    name: 'Microsoft Copilot',
    cat: 'AI Assistants',
    emoji: '🪟',
    bg: 'linear-gradient(135deg,#0078d4,#50e6ff)',
    pricing: 'freemium',
    badge: '',
    url: 'https://copilot.microsoft.com',
    desc: 'Microsoft\'s AI companion powered by GPT-4 and integrated across Windows, Edge, Microsoft 365 and Bing.',
    rating: 4.6,
    reviews: '18.7k',
    tags: [
      'Microsoft 365',
      'Windows',
      'Bing',
      'GPT-4'
    ],
    features: [
      'Free GPT-4 access in Edge & Bing',
      'Deep Microsoft 365 integration',
      'DALL-E 3 image generation',
      'Real-time web search',
      'Copilot in Word, Excel, PowerPoint'
    ],
    alts: [
      {n:'ChatGPT',t:'Freemium'},
      {n:'Gemini',t:'Free'},
      {n:'Claude',t:'Freemium'}
    ],
    price: 'Free / $30/user/mo (M365)',
    freeAlt: 'ChatGPT'
  },
  {
    id: 5,
    name: 'DeepSeek',
    cat: 'AI Assistants',
    emoji: '🐋',
    bg: 'linear-gradient(135deg,#1a56db,#0e3a8a)',
    pricing: 'free',
    badge: 'new',
    url: 'https://chat.deepseek.com',
    desc: 'China\'s breakthrough open-source AI that rivals GPT-4 at a fraction of the cost. DeepSeek R1 excels at math, coding and reasoning.',
    rating: 4.8,
    reviews: '14.3k',
    tags: [
      'open-source',
      'reasoning',
      'math',
      'coding'
    ],
    features: [
      'DeepSeek R1 reasoning model',
      'Completely free to use',
      'Long-context 128K tokens',
      'Exceptional math & coding',
      'Open weights available on HuggingFace'
    ],
    alts: [
      {n:'ChatGPT',t:'Freemium'},
      {n:'Claude',t:'Freemium'},
      {n:'Qwen',t:'Free'}
    ],
    price: 'Free',
    freeAlt: 'DeepSeek'
  },
  {
    id: 6,
    name: 'Grok',
    cat: 'AI Assistants',
    emoji: '🚀',
    bg: 'linear-gradient(135deg,#1d9bf0,#000000)',
    pricing: 'freemium',
    badge: 'new',
    url: 'https://grok.com',
    desc: 'xAI\'s Grok has real-time X (Twitter) data access, no-filter personality, and Aurora image generation. Built by Elon Musk\'s team.',
    rating: 4.6,
    reviews: '9.8k',
    tags: [
      'real-time X',
      'Aurora image gen',
      'uncensored',
      'xAI'
    ],
    features: [
      'Real-time X/Twitter data access',
      'Aurora text-to-image generation',
      'Grok 3 with deep reasoning',
      'Voice mode',
      'Free with X Premium'
    ],
    alts: [
      {n:'ChatGPT',t:'Freemium'},
      {n:'Claude',t:'Freemium'},
      {n:'Gemini',t:'Freemium'}
    ],
    price: 'Free (limited) / X Premium $8/mo',
    freeAlt: 'ChatGPT'
  },
  {
    id: 7,
    name: 'Kimi',
    cat: 'AI Assistants',
    emoji: '🌙',
    bg: 'linear-gradient(135deg,#6366f1,#8b5cf6)',
    pricing: 'freemium',
    badge: 'new',
    url: 'https://kimi.moonshot.cn',
    desc: 'Moonshot AI\'s Kimi offers a 1M token context window — the longest available — ideal for analyzing entire codebases or books.',
    rating: 4.5,
    reviews: '5.2k',
    tags: [
      '1M context',
      'long document',
      'Moonshot',
      'multilingual'
    ],
    features: [
      '1M token context window',
      'Upload entire books & codebases',
      'Bilingual Chinese/English',
      'Web search integration',
      'Free tier available'
    ],
    alts: [
      {n:'Claude',t:'Freemium'},
      {n:'Gemini',t:'Freemium'},
      {n:'ChatGPT',t:'Freemium'}
    ],
    price: 'Free / Premium CNY 99/mo',
    freeAlt: 'Gemini'
  },
  {
    id: 8,
    name: 'Qwen',
    cat: 'AI Assistants',
    emoji: '☁️',
    bg: 'linear-gradient(135deg,#7c3aed,#5b21b6)',
    pricing: 'free',
    badge: 'new',
    url: 'https://chat.qwen.ai',
    desc: 'Alibaba\'s Qwen 2.5 family rivals top models on benchmarks. Excellent multilingual support across 100+ languages with open weights.',
    rating: 4.6,
    reviews: '7.1k',
    tags: [
      'Alibaba',
      'open-source',
      'multilingual',
      'coding'
    ],
    features: [
      'Qwen 2.5-Max & Coder models',
      '100+ language support',
      'Open weights on HuggingFace',
      '128K context length',
      'Strong math & coding benchmarks'
    ],
    alts: [
      {n:'DeepSeek',t:'Free'},
      {n:'Llama',t:'Free'},
      {n:'ChatGPT',t:'Freemium'}
    ],
    price: 'Free',
    freeAlt: 'DeepSeek'
  },
  {
    id: 9,
    name: 'Llama (Meta AI)',
    cat: 'AI Assistants',
    emoji: '🦙',
    bg: 'linear-gradient(135deg,#1877f2,#0a5fce)',
    pricing: 'free',
    badge: 'open',
    url: 'https://www.meta.ai',
    desc: 'Meta\'s open-source Llama 3 models are free to download, fine-tune and deploy. Meta AI chat is integrated in WhatsApp, Instagram and Facebook.',
    rating: 4.7,
    reviews: '19.4k',
    tags: [
      'open-source',
      'Meta',
      'WhatsApp',
      'self-host'
    ],
    features: [
      'Llama 3.1 up to 405B parameters',
      'Fully open weights & commercial license',
      'Meta AI in WhatsApp/Instagram',
      'Run locally with Ollama/LM Studio',
      'Strong coding & multilingual support'
    ],
    alts: [
      {n:'Mistral',t:'Free'},
      {n:'DeepSeek',t:'Free'},
      {n:'Gemma',t:'Free'}
    ],
    price: 'Free (open-source)',
    freeAlt: 'Llama'
  },
  {
    id: 10,
    name: 'Mistral AI',
    cat: 'AI Assistants',
    emoji: '🌪️',
    bg: 'linear-gradient(135deg,#ff7000,#ff4500)',
    pricing: 'freemium',
    badge: 'open',
    url: 'https://chat.mistral.ai',
    desc: 'Europe\'s leading AI lab. Mistral models are lightweight, fast and open — from the tiny 7B to the powerful Mistral Large 2.',
    rating: 4.6,
    reviews: '8.3k',
    tags: [
      'open-source',
      'efficient',
      'European',
      'Mixtral'
    ],
    features: [
      'Mixtral 8x22B MoE architecture',
      'Le Chat free assistant',
      '32K context window',
      'Function calling & JSON mode',
      'API with competitive pricing'
    ],
    alts: [
      {n:'Llama',t:'Free'},
      {n:'DeepSeek',t:'Free'},
      {n:'Claude',t:'Freemium'}
    ],
    price: 'Free (open) / API pay-per-token',
    freeAlt: 'Llama'
  },
  {
    id: 11,
    name: 'Gemma (Google)',
    cat: 'AI Assistants',
    emoji: '💎',
    bg: 'linear-gradient(135deg,#34a853,#188038)',
    pricing: 'free',
    badge: 'open',
    url: 'https://ai.google.dev/gemma',
    desc: 'Google\'s lightweight open-source model family. Gemma 2 runs on laptops and phones while delivering impressive benchmark scores.',
    rating: 4.4,
    reviews: '4.1k',
    tags: [
      'open-source',
      'on-device',
      'lightweight',
      'Google'
    ],
    features: [
      'Runs on consumer hardware',
      'Gemma 2B, 7B & 27B variants',
      'Optimized for mobile devices',
      'Fine-tunable on personal data',
      'Apache 2.0 license'
    ],
    alts: [
      {n:'Llama',t:'Free'},
      {n:'Phi-4',t:'Free'},
      {n:'Mistral',t:'Free'}
    ],
    price: 'Free (open-source)',
    freeAlt: 'Llama'
  },
  {
    id: 12,
    name: 'Phi-4 (Microsoft)',
    cat: 'AI Assistants',
    emoji: '🔷',
    bg: 'linear-gradient(135deg,#0078d4,#004578)',
    pricing: 'free',
    badge: 'open',
    url: 'https://huggingface.co/microsoft/phi-4',
    desc: 'Microsoft\'s surprisingly capable small language model. Phi-4 punches far above its weight class on reasoning and STEM tasks.',
    rating: 4.4,
    reviews: '3.2k',
    tags: [
      'small model',
      'STEM',
      'efficient',
      'Microsoft'
    ],
    features: [
      '14B parameters only',
      'Exceptional reasoning for its size',
      'MIT license — fully commercial',
      'Runs on consumer GPUs',
      'Strong math & science benchmarks'
    ],
    alts: [
      {n:'Gemma',t:'Free'},
      {n:'Llama',t:'Free'},
      {n:'Mistral',t:'Free'}
    ],
    price: 'Free (open-source)',
    freeAlt: 'Gemma'
  },
  {
    id: 13,
    name: 'Pi (Inflection)',
    cat: 'AI Assistants',
    emoji: '🥧',
    bg: 'linear-gradient(135deg,#f97316,#dc2626)',
    pricing: 'free',
    badge: '',
    url: 'https://pi.ai',
    desc: 'Inflection\'s Pi focuses on emotional intelligence and meaningful conversations. Your personal AI friend that listens and supports.',
    rating: 4.3,
    reviews: '4.7k',
    tags: [
      'emotional AI',
      'conversational',
      'supportive',
      'personal'
    ],
    features: [
      'Empathetic conversational AI',
      'Remembers your preferences',
      'Motivational & personal coaching',
      'Voice mode for natural chat',
      'Available on iOS & Android'
    ],
    alts: [
      {n:'ChatGPT',t:'Freemium'},
      {n:'Claude',t:'Freemium'},
      {n:'Gemini',t:'Freemium'}
    ],
    price: 'Free',
    freeAlt: 'Pi'
  },
  {
    id: 14,
    name: 'Perplexity AI',
    cat: 'Research',
    emoji: '🔬',
    bg: 'linear-gradient(135deg,#0ea5e9,#0284c7)',
    pricing: 'freemium',
    badge: 'best',
    url: 'https://www.perplexity.ai',
    desc: 'The AI search engine that cites every source. Perfect for research, fact-checking, and real-time answers with full references.',
    rating: 4.8,
    reviews: '8.9k',
    tags: [
      'search',
      'research',
      'citations',
      'real-time'
    ],
    features: [
      'Real-time web search + citations',
      'Pro Search with Claude & GPT-4',
      'Spaces for collaborative research',
      'Upload & analyze documents',
      'iOS & Android app'
    ],
    alts: [
      {n:'You.com',t:'Free'},
      {n:'Phind',t:'Free'},
      {n:'Consensus',t:'Freemium'}
    ],
    price: 'Free / $20/mo',
    freeAlt: 'You.com'
  },
  {
    id: 15,
    name: 'Midjourney',
    cat: 'Image Generation',
    emoji: '🎨',
    bg: 'linear-gradient(135deg,#7c3aed,#4f46e5)',
    pricing: 'paid',
    badge: 'best',
    url: 'https://www.midjourney.com',
    desc: 'The gold standard for AI art. Breathtaking painterly results with unmatched artistic range — from photorealism to abstract illustration.',
    rating: 4.9,
    reviews: '12.4k',
    tags: [
      'text-to-image',
      'art',
      'photorealistic',
      'v6'
    ],
    features: [
      'Web interface & Discord bot',
      'v6 model with photorealism',
      'Style references & weights',
      'Commercial use on paid plans',
      'Vary & zoom tools'
    ],
    alts: [
      {n:'Leonardo AI',t:'Freemium'},
      {n:'Adobe Firefly',t:'Freemium'},
      {n:'Bing Image Creator',t:'Free'}
    ],
    price: 'From $10/mo',
    freeAlt: 'Leonardo AI'
  },
  {
    id: 16,
    name: 'DALL-E 3',
    cat: 'Image Generation',
    emoji: '🖼️',
    bg: 'linear-gradient(135deg,#10b981,#059669)',
    pricing: 'freemium',
    badge: '',
    url: 'https://openai.com/dall-e-3',
    desc: 'OpenAI\'s DALL-E 3 integrated directly in ChatGPT and Microsoft Copilot. Exceptional prompt adherence and creative versatility.',
    rating: 4.7,
    reviews: '11.2k',
    tags: [
      'OpenAI',
      'ChatGPT',
      'prompt-accurate',
      'versatile'
    ],
    features: [
      'Built into ChatGPT (free tier)',
      'Exceptional prompt following',
      'Native Microsoft Copilot integration',
      'No prohibited content filter workarounds',
      'API access for developers'
    ],
    alts: [
      {n:'Midjourney',t:'Paid'},
      {n:'Adobe Firefly',t:'Freemium'},
      {n:'Bing Image Creator',t:'Free'}
    ],
    price: 'Free via ChatGPT / API pay-per-image',
    freeAlt: 'Bing Image Creator'
  },
  {
    id: 17,
    name: 'Stable Diffusion',
    cat: 'Image Generation',
    emoji: '🌊',
    bg: 'linear-gradient(135deg,#6366f1,#4338ca)',
    pricing: 'free',
    badge: 'open',
    url: 'https://stability.ai',
    desc: 'The pioneering open-source image model that changed everything. Run locally, fine-tune on your own style, total creative freedom.',
    rating: 4.7,
    reviews: '22.1k',
    tags: [
      'open-source',
      'local',
      'fine-tuning',
      'LoRA'
    ],
    features: [
      'Fully open weights & commercial use',
      'Run 100% offline on your GPU',
      'Thousands of community LoRA models',
      'SDXL, SD 3.5 & Flux variants',
      'Automatic1111 & ComfyUI UIs'
    ],
    alts: [
      {n:'Midjourney',t:'Paid'},
      {n:'Leonardo AI',t:'Freemium'},
      {n:'Bing Image Creator',t:'Free'}
    ],
    price: 'Free (self-host)',
    freeAlt: 'Stable Diffusion'
  },
  {
    id: 18,
    name: 'Adobe Firefly',
    cat: 'Image Generation',
    emoji: '✨',
    bg: 'linear-gradient(135deg,#ff6b2b,#ff4444)',
    pricing: 'freemium',
    badge: '',
    url: 'https://firefly.adobe.com',
    desc: 'Adobe\'s commercially safe AI image generator trained exclusively on licensed content — worry-free for commercial projects.',
    rating: 4.6,
    reviews: '9.3k',
    tags: [
      'commercial-safe',
      'Photoshop',
      'generative fill',
      'vector'
    ],
    features: [
      'Generative Fill & Expand in Photoshop',
      '100% commercially licensed training data',
      'Text to vector in Illustrator',
      'Firefly in Adobe Express',
      '25 free credits per month'
    ],
    alts: [
      {n:'Bing Image Creator',t:'Free'},
      {n:'Leonardo AI',t:'Freemium'},
      {n:'DALL-E 3',t:'Freemium'}
    ],
    price: 'Free (25/mo) / CC subscription',
    freeAlt: 'Bing Image Creator'
  },
  {
    id: 19,
    name: 'Leonardo AI',
    cat: 'Image Generation',
    emoji: '🦁',
    bg: 'linear-gradient(135deg,#f59e0b,#b45309)',
    pricing: 'freemium',
    badge: '',
    url: 'https://leonardo.ai',
    desc: 'Powerful AI image generation with fine-tuned models for game assets, concept art, and product design. Generous free tier.',
    rating: 4.6,
    reviews: '7.4k',
    tags: [
      'game assets',
      'concept art',
      'fine-tuning',
      'LoRA'
    ],
    features: [
      '150 free tokens per day',
      'Fine-tune your own image models',
      'Motion: image-to-video',
      'Real-time canvas generation',
      'Alchemy upscaler'
    ],
    alts: [
      {n:'Midjourney',t:'Paid'},
      {n:'Adobe Firefly',t:'Freemium'},
      {n:'Stable Diffusion',t:'Free'}
    ],
    price: 'Free / $12/mo',
    freeAlt: 'Leonardo AI'
  },
  {
    id: 20,
    name: 'Bing Image Creator',
    cat: 'Image Generation',
    emoji: '🔵',
    bg: 'linear-gradient(135deg,#0078d4,#50e6ff)',
    pricing: 'free',
    badge: '',
    url: 'https://www.bing.com/images/create',
    desc: 'Microsoft\'s free DALL-E 3 powered image creator. No subscription needed — create high-quality images instantly via Bing.',
    rating: 4.4,
    reviews: '8.1k',
    tags: [
      'free',
      'DALL-E 3',
      'Microsoft',
      'no signup'
    ],
    features: [
      'Completely free to use',
      'Powered by DALL-E 3',
      'No account required to try',
      'High resolution outputs',
      'Microsoft account for more boosts'
    ],
    alts: [
      {n:'Adobe Firefly',t:'Freemium'},
      {n:'Leonardo AI',t:'Freemium'}
    ],
    price: 'Free',
    freeAlt: 'Bing Image Creator'
  },
  {
    id: 21,
    name: 'Ideogram',
    cat: 'Image Generation',
    emoji: '💡',
    bg: 'linear-gradient(135deg,#ec4899,#be185d)',
    pricing: 'freemium',
    badge: 'new',
    url: 'https://ideogram.ai',
    desc: 'Ideogram 2.0 leads the pack for AI images with text. Perfect typography rendering inside images — a long-standing AI weakness solved.',
    rating: 4.7,
    reviews: '4.8k',
    tags: [
      'text-in-image',
      'typography',
      'logo',
      'design'
    ],
    features: [
      'Best-in-class text rendering',
      'Ideogram 2.0 with photorealism',
      'Magic Prompt enhancement',
      'Inpainting & editing',
      '25 free images per day'
    ],
    alts: [
      {n:'Midjourney',t:'Paid'},
      {n:'DALL-E 3',t:'Freemium'},
      {n:'Adobe Firefly',t:'Freemium'}
    ],
    price: 'Free / $8/mo',
    freeAlt: 'Ideogram'
  },
  {
    id: 22,
    name: 'Flux (Black Forest Labs)',
    cat: 'Image Generation',
    emoji: '⚡',
    bg: 'linear-gradient(135deg,#1f2937,#4b5563)',
    pricing: 'freemium',
    badge: 'new',
    url: 'https://blackforestlabs.ai',
    desc: 'FLUX.1 from Black Forest Labs is the new open-source king rivaling Midjourney. Outstanding prompt adherence and photorealism.',
    rating: 4.8,
    reviews: '5.6k',
    tags: [
      'open-source',
      'photorealism',
      'prompt-accurate',
      'FLUX.1'
    ],
    features: [
      'FLUX.1 [pro], [dev] & [schnell] variants',
      'Open weights for dev & schnell',
      'Exceptional prompt adherence',
      'ControlNet & LoRA support',
      'Available on Replicate, fal.ai & more'
    ],
    alts: [
      {n:'Midjourney',t:'Paid'},
      {n:'Stable Diffusion',t:'Free'},
      {n:'Leonardo AI',t:'Freemium'}
    ],
    price: 'Free (schnell) / API pay-per-image',
    freeAlt: 'Stable Diffusion'
  },
  {
    id: 23,
    name: 'Playground AI',
    cat: 'Image Generation',
    emoji: '🛝',
    bg: 'linear-gradient(135deg,#8b5cf6,#6d28d9)',
    pricing: 'freemium',
    badge: '',
    url: 'https://playground.com',
    desc: 'Playground AI combines image generation with a canvas editor. Create, mix, and edit images with an intuitive design-first interface.',
    rating: 4.4,
    reviews: '3.1k',
    tags: [
      'canvas',
      'design',
      'free',
      'editing'
    ],
    features: [
      '500 free images per day',
      'Canvas editing workflow',
      'Mix real and AI photos',
      'SDXL & proprietary models',
      'Commercial license on paid plans'
    ],
    alts: [
      {n:'Leonardo AI',t:'Freemium'},
      {n:'Adobe Firefly',t:'Freemium'}
    ],
    price: 'Free / $15/mo',
    freeAlt: 'Playground AI'
  },
  {
    id: 24,
    name: 'Runway Gen-3',
    cat: 'Video Editing',
    emoji: '🎬',
    bg: 'linear-gradient(135deg,#f43f5e,#be123c)',
    pricing: 'freemium',
    badge: 'best',
    url: 'https://runwayml.com',
    desc: 'Gen-3 Alpha sets a cinematic bar for AI video. Create fluid, high-fidelity clips from text, images, or existing footage.',
    rating: 4.7,
    reviews: '5.1k',
    tags: [
      'text-to-video',
      'inpainting',
      'Gen-3',
      'motion'
    ],
    features: [
      'Gen-3 Alpha & Turbo models',
      'Motion Brush & Director Mode',
      'Green screen & inpainting',
      'Frame Interpolation',
      '525 free credits on signup'
    ],
    alts: [
      {n:'Pika Labs',t:'Freemium'},
      {n:'Kling AI',t:'Freemium'},
      {n:'Luma Dream Machine',t:'Freemium'}
    ],
    price: 'Free / $15/mo',
    freeAlt: 'Pika Labs'
  },
  {
    id: 25,
    name: 'Pika Labs',
    cat: 'Video Editing',
    emoji: '⚡',
    bg: 'linear-gradient(135deg,#7c3aed,#5b21b6)',
    pricing: 'freemium',
    badge: '',
    url: 'https://pika.art',
    desc: 'Pika 2.0 turns text and images into compelling video clips. Easy to use with powerful controls for motion and style.',
    rating: 4.5,
    reviews: '3.8k',
    tags: [
      'text-to-video',
      'image-to-video',
      'motion',
      'social'
    ],
    features: [
      'Pika 2.0 with improved quality',
      'Image-to-video animation',
      'Lip sync feature',
      'Text-to-video from prompts',
      '250 free credits per month'
    ],
    alts: [
      {n:'Runway Gen-3',t:'Freemium'},
      {n:'Kling AI',t:'Freemium'},
      {n:'Luma',t:'Freemium'}
    ],
    price: 'Free / $8/mo',
    freeAlt: 'Pika Labs'
  },
  {
    id: 26,
    name: 'Kling AI',
    cat: 'Video Editing',
    emoji: '🎞️',
    bg: 'linear-gradient(135deg,#0f172a,#1e40af)',
    pricing: 'freemium',
    badge: 'new',
    url: 'https://klingai.com',
    desc: 'Kuaishou\'s Kling AI produces stunning 1080p, 3-minute videos with realistic physics and cinematic motion quality.',
    rating: 4.7,
    reviews: '4.2k',
    tags: [
      '1080p',
      '3min video',
      'cinematic',
      'Kuaishou'
    ],
    features: [
      'Up to 3 min video generation',
      '1080p resolution output',
      'Realistic physics simulation',
      'Camera movement controls',
      'Image-to-video mode'
    ],
    alts: [
      {n:'Runway Gen-3',t:'Freemium'},
      {n:'Sora',t:'Paid'},
      {n:'Pika Labs',t:'Freemium'}
    ],
    price: 'Free / $10/mo',
    freeAlt: 'Kling AI'
  },
  {
    id: 27,
    name: 'Sora (OpenAI)',
    cat: 'Video Editing',
    emoji: '🌐',
    bg: 'linear-gradient(135deg,#10b981,#065f46)',
    pricing: 'paid',
    badge: 'new',
    url: 'https://sora.com',
    desc: 'OpenAI\'s groundbreaking text-to-video model. Sora creates hyper-realistic videos up to 1 minute long from simple text prompts.',
    rating: 4.6,
    reviews: '6.3k',
    tags: [
      'OpenAI',
      'text-to-video',
      'photorealistic',
      '60s'
    ],
    features: [
      'Up to 60-second video generation',
      '1080p photorealistic output',
      'Storyboard & remix modes',
      'Available in ChatGPT Plus/Pro',
      'Advanced camera controls'
    ],
    alts: [
      {n:'Runway Gen-3',t:'Freemium'},
      {n:'Kling AI',t:'Freemium'},
      {n:'Pika Labs',t:'Freemium'}
    ],
    price: '$20/mo (Plus) / $200/mo (Pro)',
    freeAlt: 'Pika Labs'
  },
  {
    id: 28,
    name: 'Luma Dream Machine',
    cat: 'Video Editing',
    emoji: '🌙',
    bg: 'linear-gradient(135deg,#6366f1,#4338ca)',
    pricing: 'freemium',
    badge: '',
    url: 'https://lumalabs.ai/dream-machine',
    desc: 'Luma Dream Machine creates smooth, high-quality 5-second video clips from text or images with impressive camera movement.',
    rating: 4.5,
    reviews: '3.1k',
    tags: [
      'text-to-video',
      'image-to-video',
      'camera motion',
      '5s clips'
    ],
    features: [
      '30 free generations per month',
      'Photorealistic video quality',
      'Smooth camera movements',
      'Image-to-video animation',
      'Fast generation speed'
    ],
    alts: [
      {n:'Pika Labs',t:'Freemium'},
      {n:'Runway Gen-3',t:'Freemium'},
      {n:'Kling AI',t:'Freemium'}
    ],
    price: 'Free / $29.99/mo',
    freeAlt: 'Luma Dream Machine'
  },
  {
    id: 29,
    name: 'Descript',
    cat: 'Video Editing',
    emoji: '🎙️',
    bg: 'linear-gradient(135deg,#8b5cf6,#6d28d9)',
    pricing: 'freemium',
    badge: '',
    url: 'https://www.descript.com',
    desc: 'Edit video by editing text. Descript auto-transcribes your footage and lets you cut, overdub, and repurpose content like a document.',
    rating: 4.6,
    reviews: '5.8k',
    tags: [
      'text-based editing',
      'overdub',
      'transcription',
      'podcast'
    ],
    features: [
      'Edit video by editing transcript',
      'Overdub AI voice cloning',
      'Screen recording & webcam',
      'Underlord AI suite of tools',
      'Remove filler words with 1 click'
    ],
    alts: [
      {n:'Kapwing',t:'Freemium'},
      {n:'CapCut',t:'Free'},
      {n:'Runway',t:'Freemium'}
    ],
    price: 'Free / $24/mo',
    freeAlt: 'CapCut'
  },
  {
    id: 30,
    name: 'CapCut',
    cat: 'Video Editing',
    emoji: '✂️',
    bg: 'linear-gradient(135deg,#000000,#3d3d3d)',
    pricing: 'free',
    badge: '',
    url: 'https://www.capcut.com',
    desc: 'TikTok\'s popular video editor now with powerful AI features. Auto captions, background removal, text-to-speech and AI effects.',
    rating: 4.5,
    reviews: '14.2k',
    tags: [
      'mobile',
      'social media',
      'TikTok',
      'free'
    ],
    features: [
      'Auto captions & subtitles',
      'AI background removal',
      'Text-to-speech voice',
      'Templates for social media',
      'Available on mobile & desktop'
    ],
    alts: [
      {n:'Descript',t:'Freemium'},
      {n:'Runway',t:'Freemium'},
      {n:'Veed.io',t:'Freemium'}
    ],
    price: 'Free / $9.99/mo',
    freeAlt: 'CapCut'
  },
  {
    id: 31,
    name: 'GitHub Copilot',
    cat: 'Code Assistant',
    emoji: '💻',
    bg: 'linear-gradient(135deg,#24292f,#4f46e5)',
    pricing: 'freemium',
    badge: 'best',
    url: 'https://github.com/features/copilot',
    desc: 'The AI pair programmer trusted by millions of developers. Autocomplete, explain, refactor and review code directly in your IDE.',
    rating: 4.7,
    reviews: '18.4k',
    tags: [
      'autocomplete',
      'VS Code',
      'JetBrains',
      'code review'
    ],
    features: [
      'Multi-line intelligent autocomplete',
      'Copilot Chat in VS Code & JetBrains',
      'Copilot Workspace for planning',
      'Free for students & OSS maintainers',
      'Security vulnerability scanning'
    ],
    alts: [
      {n:'Codeium',t:'Free'},
      {n:'Tabnine',t:'Freemium'},
      {n:'Cursor',t:'Freemium'}
    ],
    price: 'Free / $10/mo',
    freeAlt: 'Codeium'
  },
  {
    id: 32,
    name: 'Cursor',
    cat: 'Code Assistant',
    emoji: '🖱️',
    bg: 'linear-gradient(135deg,#1d4ed8,#1e40af)',
    pricing: 'freemium',
    badge: 'best',
    url: 'https://cursor.sh',
    desc: 'The AI-first code editor built on VS Code. Cursor understands your entire codebase and can write, debug and refactor at scale.',
    rating: 4.8,
    reviews: '9.6k',
    tags: [
      'AI editor',
      'codebase-aware',
      'VS Code',
      'agent'
    ],
    features: [
      'Understands entire codebase context',
      'Chat, edit & compose modes',
      'Agent mode for multi-file edits',
      'Built on VS Code — all extensions work',
      'GPT-4 & Claude backends'
    ],
    alts: [
      {n:'GitHub Copilot',t:'Freemium'},
      {n:'Codeium',t:'Free'},
      {n:'Windsurf',t:'Freemium'}
    ],
    price: 'Free / $20/mo',
    freeAlt: 'Codeium'
  },
  {
    id: 33,
    name: 'Claude Code',
    cat: 'Code Assistant',
    emoji: '🖥️',
    bg: 'linear-gradient(135deg,#c084fc,#7c3aed)',
    pricing: 'paid',
    badge: 'new',
    url: 'https://claude.ai/code',
    desc: 'Anthropic\'s agentic coding tool lives in your terminal. Claude Code reads your repo, runs commands and ships complete features.',
    rating: 4.7,
    reviews: '3.4k',
    tags: [
      'terminal',
      'agentic',
      'repo-aware',
      'Anthropic'
    ],
    features: [
      'Full repo understanding',
      'Runs shell commands autonomously',
      'Writes tests & documentation',
      'Git-aware diff workflow',
      'Available via Claude Max plan'
    ],
    alts: [
      {n:'Cursor',t:'Freemium'},
      {n:'GitHub Copilot',t:'Freemium'},
      {n:'Devin',t:'Paid'}
    ],
    price: '$100/mo (Max plan)',
    freeAlt: 'Cursor'
  },
  {
    id: 34,
    name: 'Codeium',
    cat: 'Code Assistant',
    emoji: '🟢',
    bg: 'linear-gradient(135deg,#22c55e,#15803d)',
    pricing: 'free',
    badge: '',
    url: 'https://codeium.com',
    desc: 'The best free GitHub Copilot alternative. Codeium offers AI autocomplete in 70+ languages and 40+ editors — completely free.',
    rating: 4.6,
    reviews: '6.2k',
    tags: [
      'free',
      'autocomplete',
      '70+ languages',
      '40+ editors'
    ],
    features: [
      'Always free for individuals',
      '70+ programming languages',
      '40+ IDE integrations',
      'Chat assistant in editor',
      'Codebase-aware suggestions'
    ],
    alts: [
      {n:'GitHub Copilot',t:'Freemium'},
      {n:'Tabnine',t:'Freemium'},
      {n:'Cursor',t:'Freemium'}
    ],
    price: 'Free / Enterprise plans',
    freeAlt: 'Codeium'
  },
  {
    id: 35,
    name: 'Windsurf (Codeium)',
    cat: 'Code Assistant',
    emoji: '🏄',
    bg: 'linear-gradient(135deg,#06b6d4,#0891b2)',
    pricing: 'freemium',
    badge: 'new',
    url: 'https://codeium.com/windsurf',
    desc: 'Windsurf is Codeium\'s AI IDE that flows with you. Cascade agent mode makes it the most autonomous AI coding experience available.',
    rating: 4.7,
    reviews: '4.1k',
    tags: [
      'AI IDE',
      'Cascade',
      'agent',
      'agentic coding'
    ],
    features: [
      'Cascade agentic mode',
      'Full codebase awareness',
      'Built-in terminal integration',
      'Multi-file coordinated edits',
      'Free tier with 200 credits/mo'
    ],
    alts: [
      {n:'Cursor',t:'Freemium'},
      {n:'GitHub Copilot',t:'Freemium'},
      {n:'Codeium',t:'Free'}
    ],
    price: 'Free / $15/mo',
    freeAlt: 'Codeium'
  },
  {
    id: 36,
    name: 'Bolt.new',
    cat: 'Code Assistant',
    emoji: '⚡',
    bg: 'linear-gradient(135deg,#7c3aed,#4f46e5)',
    pricing: 'freemium',
    badge: 'new',
    url: 'https://bolt.new',
    desc: 'StackBlitz\'s Bolt.new builds full-stack web apps from a single prompt in your browser. No local setup required ever.',
    rating: 4.6,
    reviews: '3.7k',
    tags: [
      'full-stack',
      'browser IDE',
      'no setup',
      'web apps'
    ],
    features: [
      'Build complete apps from prompts',
      'Full Node.js runtime in browser',
      'Instant preview & deployment',
      'One-click Netlify/Vercel deploy',
      'Supports React, Vue, Svelte & more'
    ],
    alts: [
      {n:'v0 by Vercel',t:'Freemium'},
      {n:'Cursor',t:'Freemium'},
      {n:'Replit',t:'Freemium'}
    ],
    price: 'Free / $20/mo',
    freeAlt: 'Replit'
  },
  {
    id: 37,
    name: 'v0 by Vercel',
    cat: 'Code Assistant',
    emoji: '🔲',
    bg: 'linear-gradient(135deg,#000000,#444444)',
    pricing: 'freemium',
    badge: 'new',
    url: 'https://v0.dev',
    desc: 'Vercel\'s v0 generates beautiful React UI components from text or screenshots. Ship polished UI instantly with shadcn/ui.',
    rating: 4.6,
    reviews: '3.2k',
    tags: [
      'React',
      'UI generation',
      'shadcn',
      'Tailwind'
    ],
    features: [
      'Generate React + Tailwind UI',
      'Screenshot-to-code',
      'shadcn/ui component library',
      'Refine with chat instructions',
      'Deploy to Vercel with 1 click'
    ],
    alts: [
      {n:'Bolt.new',t:'Freemium'},
      {n:'Cursor',t:'Freemium'},
      {n:'GitHub Copilot',t:'Freemium'}
    ],
    price: 'Free / $20/mo',
    freeAlt: 'v0 by Vercel'
  },
  {
    id: 38,
    name: 'Replit AI',
    cat: 'Code Assistant',
    emoji: '🔄',
    bg: 'linear-gradient(135deg,#f97316,#ea580c)',
    pricing: 'freemium',
    badge: '',
    url: 'https://replit.com',
    desc: 'Code, build and deploy from anywhere. Replit AI helps you write code with Ghostwriter and deploy apps with zero configuration.',
    rating: 4.4,
    reviews: '7.8k',
    tags: [
      'cloud IDE',
      'deploy',
      'Ghostwriter',
      'collaborative'
    ],
    features: [
      'AI Ghostwriter autocomplete',
      'Deploy apps with zero config',
      'Collaborative multiplayer coding',
      '100+ language support',
      'Always-on hosting included'
    ],
    alts: [
      {n:'Bolt.new',t:'Freemium'},
      {n:'GitHub Codespaces',t:'Freemium'},
      {n:'Codeium',t:'Free'}
    ],
    price: 'Free / $20/mo',
    freeAlt: 'Replit'
  },
  {
    id: 39,
    name: 'Jasper AI',
    cat: 'Writing & Copy',
    emoji: '✍️',
    bg: 'linear-gradient(135deg,#f43f5e,#e11d48)',
    pricing: 'paid',
    badge: '',
    url: 'https://www.jasper.ai',
    desc: 'Enterprise-grade AI writing platform. Consistent brand voice, multi-channel content generation and team collaboration at scale.',
    rating: 4.5,
    reviews: '7.8k',
    tags: [
      'copywriting',
      'brand voice',
      'SEO',
      'enterprise'
    ],
    features: [
      'Brand Voice training & consistency',
      '50+ writing templates',
      'SEO Mode with Surfer integration',
      'Multi-language support (30+)',
      'Team workflows & approvals'
    ],
    alts: [
      {n:'Copy.ai',t:'Freemium'},
      {n:'Writesonic',t:'Freemium'},
      {n:'ChatGPT',t:'Freemium'}
    ],
    price: 'From $49/mo',
    freeAlt: 'Copy.ai'
  },
  {
    id: 40,
    name: 'Copy.ai',
    cat: 'Writing & Copy',
    emoji: '📋',
    bg: 'linear-gradient(135deg,#8b5cf6,#7c3aed)',
    pricing: 'freemium',
    badge: '',
    url: 'https://www.copy.ai',
    desc: 'AI-powered copywriting for marketing teams. Generate ads, emails, blog posts and product descriptions in seconds.',
    rating: 4.4,
    reviews: '5.3k',
    tags: [
      'copywriting',
      'marketing',
      'ads',
      'emails'
    ],
    features: [
      '2,000 words free per month',
      '90+ content templates',
      'Workflows for multi-step tasks',
      'Brand voice settings',
      'Team collaboration tools'
    ],
    alts: [
      {n:'Jasper',t:'Paid'},
      {n:'Writesonic',t:'Freemium'},
      {n:'ChatGPT',t:'Freemium'}
    ],
    price: 'Free / $49/mo',
    freeAlt: 'Copy.ai'
  },
  {
    id: 41,
    name: 'Writesonic',
    cat: 'Writing & Copy',
    emoji: '📡',
    bg: 'linear-gradient(135deg,#06b6d4,#0284c7)',
    pricing: 'freemium',
    badge: '',
    url: 'https://writesonic.com',
    desc: 'Full-stack AI writing suite with SEO optimization. Writesonic includes Chatsonic (web-connected chat) and Botsonic for chatbots.',
    rating: 4.4,
    reviews: '4.9k',
    tags: [
      'SEO writing',
      'Chatsonic',
      'blog',
      'ads'
    ],
    features: [
      'Chatsonic with real-time web data',
      'Botsonic chatbot builder',
      'SEO-optimized article writer',
      'Bulk content generation',
      'Factual article mode'
    ],
    alts: [
      {n:'Jasper',t:'Paid'},
      {n:'Copy.ai',t:'Freemium'},
      {n:'ChatGPT',t:'Freemium'}
    ],
    price: 'Free / $16/mo',
    freeAlt: 'Writesonic'
  },
  {
    id: 42,
    name: 'Grammarly',
    cat: 'Writing & Copy',
    emoji: '✅',
    bg: 'linear-gradient(135deg,#15a272,#0e7a57)',
    pricing: 'freemium',
    badge: '',
    url: 'https://www.grammarly.com',
    desc: 'The world\'s leading writing assistant. Now with generative AI for drafts, rewrites, and tone adjustment across every app.',
    rating: 4.7,
    reviews: '31.4k',
    tags: [
      'grammar',
      'tone',
      'proofreading',
      'browser extension'
    ],
    features: [
      'Real-time grammar & style fixes',
      'Generative AI for rewrites',
      'Tone detection & adjustment',
      'Works in 500,000+ apps',
      'Plagiarism detection on Premium'
    ],
    alts: [
      {n:'ProWritingAid',t:'Freemium'},
      {n:'Hemingway App',t:'Free'},
      {n:'Claude',t:'Freemium'}
    ],
    price: 'Free / $12/mo',
    freeAlt: 'Grammarly'
  },
  {
    id: 43,
    name: 'Notion AI',
    cat: 'Notes & PKM',
    emoji: '📝',
    bg: 'linear-gradient(135deg,#1f2937,#4b5563)',
    pricing: 'paid',
    badge: '',
    url: 'https://www.notion.so',
    desc: 'Notion\'s built-in AI layer transforms your workspace into a smart second brain. Draft, summarize, translate and autofill databases.',
    rating: 4.5,
    reviews: '11.2k',
    tags: [
      'notes',
      'productivity',
      'docs',
      'workspace'
    ],
    features: [
      'AI writing inside any page',
      'Summarize & translate content',
      'Meeting action item extraction',
      'Database AI autofill',
      'Connectors to Slack & Google Drive'
    ],
    alts: [
      {n:'Obsidian + plugins',t:'Free'},
      {n:'Mem.ai',t:'Freemium'},
      {n:'Bear',t:'Freemium'}
    ],
    price: '+$10/seat/mo add-on',
    freeAlt: 'Obsidian'
  },
  {
    id: 44,
    name: 'ElevenLabs',
    cat: 'Voice & Speech',
    emoji: '🎙️',
    bg: 'linear-gradient(135deg,#f59e0b,#d97706)',
    pricing: 'freemium',
    badge: 'best',
    url: 'https://elevenlabs.io',
    desc: 'Hyper-realistic AI voices in 32 languages. Clone any voice from 1 minute of audio. The industry benchmark for text-to-speech.',
    rating: 4.9,
    reviews: '6.7k',
    tags: [
      'voice cloning',
      'TTS',
      'multilingual',
      'dubbing'
    ],
    features: [
      '120+ curated AI voices',
      'Voice cloning from 60s of audio',
      'Dubbing Studio for video',
      '29 languages supported',
      'API access on all plans'
    ],
    alts: [
      {n:'PlayHT',t:'Freemium'},
      {n:'Murf.ai',t:'Freemium'},
      {n:'Kokoro TTS',t:'Open Source'}
    ],
    price: 'Free / $5/mo',
    freeAlt: 'PlayHT'
  },
  {
    id: 45,
    name: 'Whisper (OpenAI)',
    cat: 'Voice & Speech',
    emoji: '🗣️',
    bg: 'linear-gradient(135deg,#10b981,#047857)',
    pricing: 'free',
    badge: 'open',
    url: 'https://openai.com/research/whisper',
    desc: 'OpenAI\'s open-source speech recognition model. Whisper transcribes audio in 99 languages with near-human accuracy.',
    rating: 4.8,
    reviews: '12.3k',
    tags: [
      'transcription',
      'open-source',
      '99 languages',
      'ASR'
    ],
    features: [
      'Near-human transcription accuracy',
      '99 language support',
      'Runs fully offline',
      'Available via OpenAI API',
      'Community UIs: Whisper Web, etc.'
    ],
    alts: [
      {n:'ElevenLabs',t:'Freemium'},
      {n:'Deepgram',t:'Freemium'},
      {n:'AssemblyAI',t:'Freemium'}
    ],
    price: 'Free (open-source) / API $0.006/min',
    freeAlt: 'Whisper'
  },
  {
    id: 46,
    name: 'Murf AI',
    cat: 'Voice & Speech',
    emoji: '🎤',
    bg: 'linear-gradient(135deg,#8b5cf6,#6d28d9)',
    pricing: 'freemium',
    badge: '',
    url: 'https://murf.ai',
    desc: 'Professional AI voice generator for videos, podcasts and e-learning. 120+ natural voices in 20+ languages with studio controls.',
    rating: 4.5,
    reviews: '3.8k',
    tags: [
      'TTS',
      'voice over',
      'e-learning',
      'podcast'
    ],
    features: [
      '120+ AI voices in 20 languages',
      'Voice changer & pitch control',
      'Sync voice to video timeline',
      'Team collaboration workspace',
      'API for developers'
    ],
    alts: [
      {n:'ElevenLabs',t:'Freemium'},
      {n:'PlayHT',t:'Freemium'},
      {n:'Descript',t:'Freemium'}
    ],
    price: 'Free / $29/mo',
    freeAlt: 'PlayHT'
  },
  {
    id: 47,
    name: 'Suno AI',
    cat: 'Audio & Music',
    emoji: '🎵',
    bg: 'linear-gradient(135deg,#a855f7,#6d28d9)',
    pricing: 'freemium',
    badge: 'new',
    url: 'https://suno.ai',
    desc: 'Generate complete songs — lyrics, vocals, instruments — from a simple text prompt. The most impressive AI music tool.',
    rating: 4.7,
    reviews: '4.2k',
    tags: [
      'music generation',
      'lyrics',
      'vocals',
      'full songs'
    ],
    features: [
      'Full song generation with vocals',
      '50+ genres supported',
      'Extend & continue songs',
      'Commercial license on paid plans',
      'Up to 4 minutes per song'
    ],
    alts: [
      {n:'Udio',t:'Freemium'},
      {n:'Stable Audio',t:'Freemium'},
      {n:'Beatoven.ai',t:'Freemium'}
    ],
    price: 'Free / $8/mo',
    freeAlt: 'Udio'
  },
  {
    id: 48,
    name: 'Udio',
    cat: 'Audio & Music',
    emoji: '🎶',
    bg: 'linear-gradient(135deg,#ec4899,#be185d)',
    pricing: 'freemium',
    badge: '',
    url: 'https://www.udio.com',
    desc: 'Udio creates studio-quality music from text prompts with exceptional genre diversity. Rivals Suno with distinct sonic character.',
    rating: 4.6,
    reviews: '3.1k',
    tags: [
      'music generation',
      'studio quality',
      'vocals',
      'genres'
    ],
    features: [
      'Studio-quality audio output',
      'Diverse genre & style range',
      'Extend & remix tracks',
      'Lyrics generation',
      'Commercial licensing available'
    ],
    alts: [
      {n:'Suno AI',t:'Freemium'},
      {n:'Stable Audio',t:'Freemium'},
      {n:'Beatoven',t:'Freemium'}
    ],
    price: 'Free / $10/mo',
    freeAlt: 'Suno AI'
  },
  {
    id: 49,
    name: 'Canva AI',
    cat: 'Graphic Design',
    emoji: '🎭',
    bg: 'linear-gradient(135deg,#06b6d4,#0891b2)',
    pricing: 'freemium',
    badge: 'best',
    url: 'https://www.canva.com',
    desc: 'Drag-and-drop design elevated by AI. Magic Studio adds instant image generation, background removal, and smart resize.',
    rating: 4.6,
    reviews: '24.8k',
    tags: [
      'design',
      'templates',
      'social media',
      'Magic Studio'
    ],
    features: [
      'Magic Write & Magic Design',
      'Text to Image generation',
      'Background Remover',
      'Brand Kit & team collaboration',
      '100GB cloud storage'
    ],
    alts: [
      {n:'Adobe Express',t:'Free'},
      {n:'Microsoft Designer',t:'Free'},
      {n:'Looka',t:'Freemium'}
    ],
    price: 'Free / $15/mo',
    freeAlt: 'Microsoft Designer'
  },
  {
    id: 50,
    name: 'Microsoft Designer',
    cat: 'Graphic Design',
    emoji: '🎨',
    bg: 'linear-gradient(135deg,#0078d4,#7719aa)',
    pricing: 'free',
    badge: '',
    url: 'https://designer.microsoft.com',
    desc: 'Microsoft\'s free AI design tool powered by DALL-E 3. Create social posts, presentations and marketing graphics in seconds.',
    rating: 4.3,
    reviews: '4.2k',
    tags: [
      'free',
      'Microsoft',
      'DALL-E 3',
      'social media'
    ],
    features: [
      'Completely free AI design tool',
      'DALL-E 3 image generation',
      'Smart design suggestions',
      'Integrated with Microsoft 365',
      'Resize for any platform'
    ],
    alts: [
      {n:'Canva AI',t:'Freemium'},
      {n:'Adobe Express',t:'Freemium'},
      {n:'Looka',t:'Freemium'}
    ],
    price: 'Free',
    freeAlt: 'Microsoft Designer'
  },
  {
    id: 51,
    name: 'Figma AI',
    cat: 'Graphic Design',
    emoji: '🔷',
    bg: 'linear-gradient(135deg,#f24e1e,#ff7262)',
    pricing: 'freemium',
    badge: 'new',
    url: 'https://www.figma.com',
    desc: 'Figma\'s AI features accelerate UI/UX design with auto-layout, first draft generation, and design-to-code capabilities.',
    rating: 4.7,
    reviews: '9.3k',
    tags: [
      'UI/UX',
      'design tool',
      'prototyping',
      'collaboration'
    ],
    features: [
      'AI first draft from description',
      'Auto Layout & Variables',
      'Dev Mode for design-to-code',
      'FigJam AI for brainstorming',
      'Real-time collaborative design'
    ],
    alts: [
      {n:'Canva AI',t:'Freemium'},
      {n:'Framer AI',t:'Freemium'},
      {n:'Penpot',t:'Free'}
    ],
    price: 'Free / $15/mo',
    freeAlt: 'Penpot'
  },
  {
    id: 52,
    name: 'HeyGen',
    cat: 'Video Avatars',
    emoji: '📹',
    bg: 'linear-gradient(135deg,#06b6d4,#0ea5e9)',
    pricing: 'freemium',
    badge: '',
    url: 'https://www.heygen.com',
    desc: 'Create professional AI avatar videos in minutes. Translate videos into 40+ languages with perfect lip-sync.',
    rating: 4.6,
    reviews: '3.9k',
    tags: [
      'AI avatars',
      'video translation',
      'lip-sync',
      'presenter'
    ],
    features: [
      '300+ AI avatars available',
      'Video translation in 40+ languages',
      'Instant avatar from photo',
      'Custom branded avatars',
      'API access for automation'
    ],
    alts: [
      {n:'Synthesia',t:'Paid'},
      {n:'D-ID',t:'Freemium'},
      {n:'Colossyan',t:'Paid'}
    ],
    price: 'Free / $24/mo',
    freeAlt: 'D-ID'
  },
  {
    id: 53,
    name: 'Synthesia',
    cat: 'Video Avatars',
    emoji: '🎭',
    bg: 'linear-gradient(135deg,#8b5cf6,#4f46e5)',
    pricing: 'paid',
    badge: '',
    url: 'https://www.synthesia.io',
    desc: 'Enterprise AI video platform with 160+ AI avatars. Create training, marketing and explainer videos in 120+ languages.',
    rating: 4.5,
    reviews: '4.6k',
    tags: [
      'enterprise',
      '160+ avatars',
      'training',
      '120 languages'
    ],
    features: [
      '160+ diverse AI avatars',
      '120+ languages & accents',
      'Custom avatar creation',
      'Brand kit & templates',
      'SCORM export for LMS'
    ],
    alts: [
      {n:'HeyGen',t:'Freemium'},
      {n:'D-ID',t:'Freemium'},
      {n:'Colossyan',t:'Paid'}
    ],
    price: 'From $29/mo',
    freeAlt: 'D-ID'
  },
  {
    id: 54,
    name: 'Consensus',
    cat: 'Research',
    emoji: '🔭',
    bg: 'linear-gradient(135deg,#0ea5e9,#0369a1)',
    pricing: 'freemium',
    badge: '',
    url: 'https://consensus.app',
    desc: 'AI search engine built exclusively for scientific research. Find peer-reviewed evidence and consensus across 200M+ papers.',
    rating: 4.6,
    reviews: '3.4k',
    tags: [
      'scientific',
      'peer-reviewed',
      'citations',
      'academia'
    ],
    features: [
      '200M+ scientific papers indexed',
      'Consensus Meter for agreement level',
      'Copilot for research summaries',
      'Filter by year, journal, study type',
      'Citation export to Zotero/Mendeley'
    ],
    alts: [
      {n:'Perplexity',t:'Freemium'},
      {n:'Elicit',t:'Freemium'},
      {n:'Semantic Scholar',t:'Free'}
    ],
    price: 'Free / $10.99/mo',
    freeAlt: 'Semantic Scholar'
  },
  {
    id: 55,
    name: 'NotebookLM',
    cat: 'Research',
    emoji: '📓',
    bg: 'linear-gradient(135deg,#4285f4,#0f9d58)',
    pricing: 'free',
    badge: 'new',
    url: 'https://notebooklm.google',
    desc: 'Google\'s AI research assistant lets you upload documents and have a conversation with them. Groundbreaking Audio Overview feature.',
    rating: 4.8,
    reviews: '7.1k',
    tags: [
      'document analysis',
      'Google',
      'audio overview',
      'sources'
    ],
    features: [
      'Upload 50 sources (PDF, docs, etc.)',
      'Audio Overview podcast generation',
      'Grounded only in your sources',
      'Mind maps & study guides',
      'Completely free to use'
    ],
    alts: [
      {n:'Perplexity',t:'Freemium'},
      {n:'ChatPDF',t:'Freemium'},
      {n:'Claude',t:'Freemium'}
    ],
    price: 'Free',
    freeAlt: 'NotebookLM'
  },
  {
    id: 56,
    name: 'You.com',
    cat: 'Research',
    emoji: '🔍',
    bg: 'linear-gradient(135deg,#6366f1,#4338ca)',
    pricing: 'freemium',
    badge: '',
    url: 'https://you.com',
    desc: 'AI-powered search engine that lets you choose your AI model (GPT-4, Claude, Gemini) for answering questions with web citations.',
    rating: 4.4,
    reviews: '4.2k',
    tags: [
      'AI search',
      'multi-model',
      'citations',
      'privacy'
    ],
    features: [
      'Choose GPT-4, Claude or Gemini',
      'Web-grounded answers with citations',
      'YouCode for developer queries',
      'Privacy-focused mode',
      'Free AI models with ads'
    ],
    alts: [
      {n:'Perplexity',t:'Freemium'},
      {n:'Phind',t:'Free'},
      {n:'Bing AI',t:'Free'}
    ],
    price: 'Free / $15/mo',
    freeAlt: 'You.com'
  },
  {
    id: 57,
    name: 'Zapier AI',
    cat: 'Automation',
    emoji: '⚡',
    bg: 'linear-gradient(135deg,#ff4a00,#ff6b00)',
    pricing: 'freemium',
    badge: '',
    url: 'https://zapier.com',
    desc: 'Connect 7,000+ apps with AI-powered automation. Zapier\'s AI Zap builder creates workflows from plain English descriptions.',
    rating: 4.6,
    reviews: '9.2k',
    tags: [
      'automation',
      '7000+ apps',
      'workflows',
      'no-code'
    ],
    features: [
      '7,000+ app integrations',
      'AI Zap builder from plain English',
      'Multi-step Zap workflows',
      'Zapier Tables & Interfaces',
      '5 free Zaps forever'
    ],
    alts: [
      {n:'Make (Integromat)',t:'Freemium'},
      {n:'n8n',t:'Free'},
      {n:'Pabbly Connect',t:'Paid'}
    ],
    price: 'Free / $19.99/mo',
    freeAlt: 'n8n'
  },
  {
    id: 58,
    name: 'Make (Integromat)',
    cat: 'Automation',
    emoji: '🔧',
    bg: 'linear-gradient(135deg,#7c3aed,#5b21b6)',
    pricing: 'freemium',
    badge: '',
    url: 'https://www.make.com',
    desc: 'Visual automation platform more powerful than Zapier. Make connects 1,000+ apps with complex multi-branch workflows.',
    rating: 4.6,
    reviews: '6.1k',
    tags: [
      'visual automation',
      '1000+ apps',
      'workflows',
      'advanced'
    ],
    features: [
      'Visual drag-and-drop builder',
      '1,000+ app connectors',
      'Complex branching & routing',
      'HTTP & JSON modules',
      '1,000 free ops/month'
    ],
    alts: [
      {n:'Zapier',t:'Freemium'},
      {n:'n8n',t:'Free'},
      {n:'Pabbly',t:'Paid'}
    ],
    price: 'Free / $9/mo',
    freeAlt: 'n8n'
  },
  {
    id: 59,
    name: 'n8n',
    cat: 'Automation',
    emoji: '🔗',
    bg: 'linear-gradient(135deg,#ea4b71,#c2185b)',
    pricing: 'free',
    badge: 'open',
    url: 'https://n8n.io',
    desc: 'Open-source workflow automation you can self-host. n8n gives you the power of Zapier with full data privacy and no limits.',
    rating: 4.7,
    reviews: '5.3k',
    tags: [
      'open-source',
      'self-host',
      'workflows',
      'privacy'
    ],
    features: [
      '400+ integrations built-in',
      'Self-host for full data control',
      'AI Agent nodes built-in',
      'Code nodes for custom logic',
      'Unlimited workflows on self-host'
    ],
    alts: [
      {n:'Zapier',t:'Freemium'},
      {n:'Make',t:'Freemium'},
      {n:'Activepieces',t:'Free'}
    ],
    price: 'Free (self-host) / $20/mo cloud',
    freeAlt: 'n8n'
  },
  {
    id: 60,
    name: 'Semrush AI',
    cat: 'SEO & Marketing',
    emoji: '📊',
    bg: 'linear-gradient(135deg,#ff6b35,#f7c59f)',
    pricing: 'paid',
    badge: 'best',
    url: 'https://www.semrush.com',
    desc: 'The all-in-one SEO & marketing intelligence platform. AI features include content briefs, keyword clusters and competitor insights.',
    rating: 4.7,
    reviews: '12.1k',
    tags: [
      'SEO',
      'keywords',
      'competitor analysis',
      'content'
    ],
    features: [
      'Keyword Magic Tool (20B+ keywords)',
      'AI Writing Assistant for SEO',
      'Site audit & technical SEO',
      'Backlink gap analysis',
      'Social media management'
    ],
    alts: [
      {n:'Ahrefs',t:'Paid'},
      {n:'Surfer SEO',t:'Paid'},
      {n:'NeuronWriter',t:'Freemium'}
    ],
    price: 'From $129/mo',
    freeAlt: 'Ubersuggest'
  },
  {
    id: 61,
    name: 'Surfer SEO',
    cat: 'SEO & Marketing',
    emoji: '🏄',
    bg: 'linear-gradient(135deg,#0ea5e9,#0284c7)',
    pricing: 'paid',
    badge: '',
    url: 'https://surferseo.com',
    desc: 'Data-driven on-page SEO optimization tool. Surfer analyzes top-ranking pages and gives a real-time content score as you write.',
    rating: 4.6,
    reviews: '5.7k',
    tags: [
      'on-page SEO',
      'content score',
      'SERP analysis',
      'NLP'
    ],
    features: [
      'Real-time content editor with score',
      'SERP analyzer for any keyword',
      'Content planner & clustering',
      'Jasper & Google Docs integration',
      'AI Article writer'
    ],
    alts: [
      {n:'Semrush',t:'Paid'},
      {n:'NeuronWriter',t:'Freemium'},
      {n:'Frase',t:'Paid'}
    ],
    price: 'From $89/mo',
    freeAlt: 'NeuronWriter'
  },
  {
    id: 62,
    name: 'Adobe Photoshop AI',
    cat: 'Photo Editing',
    emoji: '🖼️',
    bg: 'linear-gradient(135deg,#001e36,#31a8ff)',
    pricing: 'paid',
    badge: 'best',
    url: 'https://www.adobe.com/products/photoshop',
    desc: 'Photoshop\'s Generative Fill is a paradigm shift. Add, remove or expand anything in photos with AI that matches lighting and texture.',
    rating: 4.8,
    reviews: '19.4k',
    tags: [
      'generative fill',
      'remove object',
      'extend photo',
      'Firefly'
    ],
    features: [
      'Generative Fill & Expand',
      'Remove & replace objects with AI',
      'Neural Filters for portraits',
      'Content-Aware Fill',
      'AI-powered selection tools'
    ],
    alts: [
      {n:'Canva AI',t:'Freemium'},
      {n:'Luminar Neo',t:'Paid'},
      {n:'Fotor',t:'Freemium'}
    ],
    price: '$20.99/mo (CC)',
    freeAlt: 'Canva AI'
  },
  {
    id: 63,
    name: 'Luminar Neo',
    cat: 'Photo Editing',
    emoji: '⭐',
    bg: 'linear-gradient(135deg,#1e3a5f,#4a90d9)',
    pricing: 'paid',
    badge: '',
    url: 'https://skylum.com/luminar-neo',
    desc: 'AI-first photo editor with one-click sky replacement, portrait retouching, and background generation powered by Skylum AI.',
    rating: 4.4,
    reviews: '4.1k',
    tags: [
      'sky replacement',
      'portrait AI',
      'background',
      'retouching'
    ],
    features: [
      'Sky AI with realistic blending',
      'Portrait Bokeh & Skin AI',
      'Background Removal & Replacement',
      'Relight AI for scene lighting',
      'Accent AI for one-click enhance'
    ],
    alts: [
      {n:'Photoshop AI',t:'Paid'},
      {n:'Topaz Photo AI',t:'Paid'},
      {n:'Canva AI',t:'Freemium'}
    ],
    price: 'From $9.95/mo',
    freeAlt: 'Canva AI'
  },
  {
    id: 64,
    name: 'Remove.bg',
    cat: 'Photo Editing',
    emoji: '🔲',
    bg: 'linear-gradient(135deg,#6366f1,#8b5cf6)',
    pricing: 'freemium',
    badge: '',
    url: 'https://www.remove.bg',
    desc: 'Remove image backgrounds in 5 seconds — automatically and for free. The most accurate background removal tool available.',
    rating: 4.7,
    reviews: '7.8k',
    tags: [
      'background removal',
      'free',
      'product photos',
      'portraits'
    ],
    features: [
      'Instant automatic background removal',
      'HD download on paid plan',
      'API for bulk processing',
      'Batch processing',
      'Integrations with Photoshop & Figma'
    ],
    alts: [
      {n:'Canva AI',t:'Freemium'},
      {n:'PhotoRoom',t:'Freemium'},
      {n:'ClipDrop',t:'Freemium'}
    ],
    price: 'Free (small) / $9/mo HD',
    freeAlt: 'Remove.bg'
  },
  {
    id: 65,
    name: 'Julius AI',
    cat: 'Data Analysis',
    emoji: '📈',
    bg: 'linear-gradient(135deg,#f59e0b,#b45309)',
    pricing: 'freemium',
    badge: '',
    url: 'https://julius.ai',
    desc: 'Julius is your AI data analyst. Upload spreadsheets, CSVs or databases and get charts, insights and Python code in plain English.',
    rating: 4.6,
    reviews: '3.2k',
    tags: [
      'data analysis',
      'charts',
      'CSV',
      'Python'
    ],
    features: [
      'Chat with your spreadsheet data',
      'Auto-generate Python & R code',
      'Create charts & visualizations',
      'Connect to databases',
      'Statistical analysis & forecasting'
    ],
    alts: [
      {n:'Rows AI',t:'Freemium'},
      {n:'Akkio',t:'Freemium'},
      {n:'ChatGPT',t:'Freemium'}
    ],
    price: 'Free / $25/mo',
    freeAlt: 'Julius AI'
  },
  {
    id: 66,
    name: 'Tableau AI',
    cat: 'Data Analysis',
    emoji: '📊',
    bg: 'linear-gradient(135deg,#e87722,#c05621)',
    pricing: 'paid',
    badge: '',
    url: 'https://www.tableau.com',
    desc: 'Industry-leading BI tool with Einstein Copilot AI. Ask questions in natural language and get instant visualizations from your data.',
    rating: 4.6,
    reviews: '11.4k',
    tags: [
      'business intelligence',
      'dashboards',
      'NLP',
      'enterprise'
    ],
    features: [
      'Einstein Copilot natural language queries',
      '300+ data source connectors',
      'AI-powered data prep',
      'Real-time dashboards',
      'Embedded analytics'
    ],
    alts: [
      {n:'Power BI',t:'Freemium'},
      {n:'Looker Studio',t:'Free'},
      {n:'Julius AI',t:'Freemium'}
    ],
    price: 'From $75/user/mo',
    freeAlt: 'Looker Studio'
  },
  {
    id: 67,
    name: 'Intercom Fin',
    cat: 'Chatbots',
    emoji: '💬',
    bg: 'linear-gradient(135deg,#0e6eff,#0052cc)',
    pricing: 'paid',
    badge: 'best',
    url: 'https://www.intercom.com/fin',
    desc: 'Fin is Intercom\'s GPT-4 powered support AI. It resolves 50%+ of support queries instantly by reading your help center.',
    rating: 4.6,
    reviews: '4.8k',
    tags: [
      'customer support',
      'GPT-4',
      'help desk',
      'automated'
    ],
    features: [
      'Resolves 50%+ queries automatically',
      'Reads your entire knowledge base',
      'Seamless human handoff',
      'Multiple language support',
      'Detailed conversation analytics'
    ],
    alts: [
      {n:'Tidio AI',t:'Freemium'},
      {n:'Crisp AI',t:'Freemium'},
      {n:'Botpress',t:'Free'}
    ],
    price: '$0.99 per resolution',
    freeAlt: 'Botpress'
  },
  {
    id: 68,
    name: 'Botpress',
    cat: 'Chatbots',
    emoji: '🤖',
    bg: 'linear-gradient(135deg,#1a1a2e,#16213e)',
    pricing: 'freemium',
    badge: '',
    url: 'https://botpress.com',
    desc: 'Open-source chatbot platform with GPT-native AI. Build, test and deploy intelligent chatbots without coding expertise.',
    rating: 4.5,
    reviews: '3.6k',
    tags: [
      'open-source',
      'chatbot builder',
      'no-code',
      'GPT-native'
    ],
    features: [
      'Visual flow builder',
      'GPT-native conversation AI',
      '100+ integrations',
      'Analytics & testing suite',
      'Free community plan'
    ],
    alts: [
      {n:'Tidio AI',t:'Freemium'},
      {n:'Voiceflow',t:'Freemium'},
      {n:'Landbot',t:'Freemium'}
    ],
    price: 'Free / $89/mo',
    freeAlt: 'Botpress'
  },
  {
    id: 69,
    name: 'ChatPDF',
    cat: 'PDF & Docs',
    emoji: '📄',
    bg: 'linear-gradient(135deg,#dc2626,#b91c1c)',
    pricing: 'freemium',
    badge: '',
    url: 'https://www.chatpdf.com',
    desc: 'Upload any PDF and chat with it. Ask questions, get summaries, and extract key information from research papers and contracts.',
    rating: 4.5,
    reviews: '5.9k',
    tags: [
      'PDF chat',
      'summarize',
      'Q&A',
      'research papers'
    ],
    features: [
      'Chat with any PDF instantly',
      'Multi-PDF conversations on paid',
      'Supports all languages',
      'Highlight key sections',
      'Free: 2 PDFs/day up to 120 pages'
    ],
    alts: [
      {n:'Humata',t:'Freemium'},
      {n:'AskYourPDF',t:'Freemium'},
      {n:'NotebookLM',t:'Free'}
    ],
    price: 'Free / $5/mo',
    freeAlt: 'NotebookLM'
  },
  {
    id: 70,
    name: 'Humata AI',
    cat: 'PDF & Docs',
    emoji: '📚',
    bg: 'linear-gradient(135deg,#7c3aed,#5b21b6)',
    pricing: 'freemium',
    badge: '',
    url: 'https://www.humata.ai',
    desc: 'Chat with your entire document library at once. Humata AI is built for researchers and legal teams handling complex documents.',
    rating: 4.5,
    reviews: '2.8k',
    tags: [
      'document analysis',
      'research',
      'legal',
      'multi-doc'
    ],
    features: [
      'Chat across multiple documents',
      'Summarize 100-page docs in seconds',
      'Compare documents side by side',
      'Citation-grounded answers',
      'Team workspace for collaboration'
    ],
    alts: [
      {n:'ChatPDF',t:'Freemium'},
      {n:'NotebookLM',t:'Free'},
      {n:'Claude',t:'Freemium'}
    ],
    price: 'Free / $14.99/mo',
    freeAlt: 'NotebookLM'
  },
  {
    id: 71,
    name: 'Instantly AI',
    cat: 'Email & Outreach',
    emoji: '📨',
    bg: 'linear-gradient(135deg,#f59e0b,#d97706)',
    pricing: 'paid',
    badge: '',
    url: 'https://instantly.ai',
    desc: 'Scale cold email outreach with unlimited accounts. AI personalizes emails and optimizes send times to maximize reply rates.',
    rating: 4.6,
    reviews: '4.3k',
    tags: [
      'cold email',
      'outreach',
      'deliverability',
      'sales'
    ],
    features: [
      'Unlimited email accounts',
      'AI-powered personalization',
      'Warm-up network for deliverability',
      'A/B testing for sequences',
      'Analytics & reply tracking'
    ],
    alts: [
      {n:'Smartlead',t:'Paid'},
      {n:'Lemlist',t:'Paid'},
      {n:'Mailmeteor',t:'Freemium'}
    ],
    price: 'From $37/mo',
    freeAlt: 'Mailmeteor'
  },
  {
    id: 72,
    name: 'Spline AI',
    cat: '3D & AR',
    emoji: '🧊',
    bg: 'linear-gradient(135deg,#6366f1,#8b5cf6)',
    pricing: 'freemium',
    badge: 'new',
    url: 'https://spline.design',
    desc: 'Create stunning 3D scenes, animations and interactive experiences right in your browser. No 3D expertise required.',
    rating: 4.5,
    reviews: '3.1k',
    tags: [
      '3D design',
      'browser-based',
      'interactive',
      'animation'
    ],
    features: [
      'AI 3D object generation',
      'Real-time collaboration',
      'Export to React & web',
      'Physics simulations',
      'Free tier available'
    ],
    alts: [
      {n:'Blender (AI plugins)',t:'Free'},
      {n:'Meshy AI',t:'Freemium'},
      {n:'Luma AI',t:'Freemium'}
    ],
    price: 'Free / $9/mo',
    freeAlt: 'Spline AI'
  },
  {
    id: 73,
    name: 'Meshy AI',
    cat: '3D & AR',
    emoji: '🎲',
    bg: 'linear-gradient(135deg,#f43f5e,#be123c)',
    pricing: 'freemium',
    badge: 'new',
    url: 'https://www.meshy.ai',
    desc: 'Generate 3D models and textures from text or images in seconds. Perfect for game developers and 3D artists.',
    rating: 4.5,
    reviews: '2.4k',
    tags: [
      '3D model generation',
      'textures',
      'game dev',
      'text-to-3D'
    ],
    features: [
      'Text-to-3D model generation',
      'Image-to-3D conversion',
      'AI texturing for existing models',
      'Export to FBX, OBJ, GLB',
      '200 free credits monthly'
    ],
    alts: [
      {n:'Spline AI',t:'Freemium'},
      {n:'Luma AI',t:'Freemium'},
      {n:'Tripo AI',t:'Freemium'}
    ],
    price: 'Free / $20/mo',
    freeAlt: 'Meshy AI'
  },
  {
    id: 74,
    name: 'Otter AI',
    cat: 'Productivity',
    emoji: '🦦',
    bg: 'linear-gradient(135deg,#0ea5e9,#0284c7)',
    pricing: 'freemium',
    badge: '',
    url: 'https://otter.ai',
    desc: 'AI meeting assistant that joins Zoom, Teams and Meet. Otter transcribes, summarizes and extracts action items automatically.',
    rating: 4.6,
    reviews: '8.4k',
    tags: [
      'meeting transcription',
      'Zoom',
      'Teams',
      'action items'
    ],
    features: [
      'Auto-joins Zoom/Teams/Meet',
      'Real-time transcription & notes',
      'AI summary & action items',
      'Speaker identification',
      '600 min/mo free'
    ],
    alts: [
      {n:'Fireflies.ai',t:'Freemium'},
      {n:'Fathom',t:'Free'},
      {n:'Tactiq',t:'Freemium'}
    ],
    price: 'Free / $16.99/mo',
    freeAlt: 'Fathom'
  },
  {
    id: 75,
    name: 'Fireflies AI',
    cat: 'Productivity',
    emoji: '🔥',
    bg: 'linear-gradient(135deg,#7c3aed,#5b21b6)',
    pricing: 'freemium',
    badge: '',
    url: 'https://fireflies.ai',
    desc: 'Fireflies joins your meetings, transcribes everything and creates searchable notes with AI summaries and topic tracking.',
    rating: 4.5,
    reviews: '6.1k',
    tags: [
      'meeting notes',
      'transcription',
      'search',
      'CRM integration'
    ],
    features: [
      'Joins any video conference',
      'Searchable meeting transcripts',
      'AI summaries & action items',
      'CRM integration (Salesforce, HubSpot)',
      '800 min storage free'
    ],
    alts: [
      {n:'Otter AI',t:'Freemium'},
      {n:'Fathom',t:'Free'},
      {n:'Avoma',t:'Paid'}
    ],
    price: 'Free / $18/mo',
    freeAlt: 'Fathom'
  },
  {
    id: 76,
    name: 'Gamma AI',
    cat: 'Productivity',
    emoji: '📊',
    bg: 'linear-gradient(135deg,#8b5cf6,#6d28d9)',
    pricing: 'freemium',
    badge: 'new',
    url: 'https://gamma.app',
    desc: 'Create beautiful presentations, documents and webpages from a single prompt. Gamma replaces PowerPoint for the AI era.',
    rating: 4.7,
    reviews: '5.8k',
    tags: [
      'presentations',
      'slides',
      'docs',
      'one-prompt'
    ],
    features: [
      'Generate full decks from a prompt',
      '40+ beautiful templates',
      'One-click restyling',
      'Embed video, GIFs and charts',
      'Analytics for shared decks'
    ],
    alts: [
      {n:'Tome AI',t:'Freemium'},
      {n:'Beautiful.ai',t:'Paid'},
      {n:'PowerPoint Copilot',t:'Paid'}
    ],
    price: 'Free / $10/mo',
    freeAlt: 'Gamma AI'
  },
  {
    id: 77,
    name: 'Loom AI',
    cat: 'Productivity',
    emoji: '🎥',
    bg: 'linear-gradient(135deg,#625df5,#4f46e5)',
    pricing: 'freemium',
    badge: '',
    url: 'https://www.loom.com',
    desc: 'Record and share screen videos instantly. Loom AI auto-generates titles, summaries and removes filler words from your recordings.',
    rating: 4.6,
    reviews: '7.2k',
    tags: [
      'screen recording',
      'async video',
      'summaries',
      'remote work'
    ],
    features: [
      'AI-generated video summaries',
      'Remove filler words automatically',
      'Auto-chapters & transcripts',
      'Engagement analytics',
      '25 videos free forever'
    ],
    alts: [
      {n:'Descript',t:'Freemium'},
      {n:'Vidyard',t:'Freemium'},
      {n:'Tella',t:'Freemium'}
    ],
    price: 'Free / $15/mo',
    freeAlt: 'Loom'
  },
  {
    id: 78,
    name: 'Character AI',
    cat: 'AI Assistants',
    emoji: '🎭',
    bg: 'linear-gradient(135deg,#7c3aed,#a855f7)',
    pricing: 'freemium',
    badge: '',
    url: 'https://character.ai',
    desc: 'Chat with AI characters based on celebrities, fictional characters and original personas. The most popular AI companion platform.',
    rating: 4.4,
    reviews: '18.6k',
    tags: [
      'character chat',
      'roleplay',
      'creative',
      'companion'
    ],
    features: [
      'Millions of community characters',
      'Create your own AI persona',
      'Group chats with multiple AIs',
      'Character.AI voice feature',
      'Available on iOS & Android'
    ],
    alts: [
      {n:'ChatGPT',t:'Freemium'},
      {n:'Claude',t:'Freemium'},
      {n:'Replika',t:'Freemium'}
    ],
    price: 'Free / $9.99/mo (no filter)',
    freeAlt: 'Character AI'
  },
  {
    id: 79,
    name: 'Devin AI',
    cat: 'Code Assistant',
    emoji: '🤖',
    bg: 'linear-gradient(135deg,#0f172a,#1e3a5f)',
    pricing: 'paid',
    badge: 'new',
    url: 'https://www.cognition.ai/blog/introducing-devin',
    desc: 'The world\'s first fully autonomous AI software engineer by Cognition AI. Devin completes entire engineering tasks end-to-end.',
    rating: 4.3,
    reviews: '2.1k',
    tags: [
      'autonomous',
      'software engineer',
      'full-stack',
      'agent'
    ],
    features: [
      'Plans & executes complete engineering tasks',
      'Uses own shell, editor & browser',
      'Learns from feedback during tasks',
      'Handles GitHub issues end-to-end',
      'Passes real engineering interview tasks'
    ],
    alts: [
      {n:'Claude Code',t:'Paid'},
      {n:'Cursor',t:'Freemium'},
      {n:'GitHub Copilot',t:'Freemium'}
    ],
    price: 'Enterprise pricing',
    freeAlt: 'Cursor'
  },
  {
    id: 80,
    name: 'Fathom',
    cat: 'Productivity',
    emoji: '🎯',
    bg: 'linear-gradient(135deg,#10b981,#059669)',
    pricing: 'free',
    badge: '',
    url: 'https://fathom.video',
    desc: 'The best free AI meeting recorder. Fathom records, transcribes and summarizes Zoom calls — completely free with no limits.',
    rating: 4.8,
    reviews: '4.3k',
    tags: [
      'free',
      'Zoom',
      'meeting notes',
      'unlimited'
    ],
    features: [
      'Completely free — unlimited recordings',
      'Instant AI summary after call',
      'Action items & follow-up email drafts',
      'CRM auto-sync (Salesforce, HubSpot)',
      'Highlights & clip sharing'
    ],
    alts: [
      {n:'Otter AI',t:'Freemium'},
      {n:'Fireflies',t:'Freemium'},
      {n:'Avoma',t:'Paid'}
    ],
    price: 'Free',
    freeAlt: 'Fathom'
  },
  {
    id: 81,
    name: 'Phind',
    cat: 'Research',
    emoji: '🔎',
    bg: 'linear-gradient(135deg,#6366f1,#4f46e5)',
    pricing: 'freemium',
    badge: '',
    url: 'https://www.phind.com',
    desc: 'AI search engine built specifically for developers. Phind searches the web and Stack Overflow to give cited, code-rich answers.',
    rating: 4.6,
    reviews: '3.8k',
    tags: [
      'developer search',
      'coding',
      'Stack Overflow',
      'citations'
    ],
    features: [
      'Web + Stack Overflow search',
      'Code-focused AI answers',
      'Phind-70B open-source model',
      'Pair programmer mode',
      'IDE extension available'
    ],
    alts: [
      {n:'Perplexity',t:'Freemium'},
      {n:'You.com',t:'Freemium'},
      {n:'GitHub Copilot',t:'Freemium'}
    ],
    price: 'Free / $17/mo',
    freeAlt: 'Phind'
  },
  {
    id: 82,
    name: 'Elicit',
    cat: 'Research',
    emoji: '🧪',
    bg: 'linear-gradient(135deg,#0ea5e9,#7c3aed)',
    pricing: 'freemium',
    badge: '',
    url: 'https://elicit.com',
    desc: 'AI research assistant for literature review. Elicit finds relevant papers, extracts key data and synthesizes findings across studies.',
    rating: 4.5,
    reviews: '2.7k',
    tags: [
      'literature review',
      'papers',
      'data extraction',
      'synthesis'
    ],
    features: [
      'Searches 125M+ academic papers',
      'Extracts structured data from tables',
      'Synthesizes findings across papers',
      'Export to CSV for analysis',
      'Ask any research question'
    ],
    alts: [
      {n:'Consensus',t:'Freemium'},
      {n:'Semantic Scholar',t:'Free'},
      {n:'ResearchRabbit',t:'Free'}
    ],
    price: 'Free / $12/mo',
    freeAlt: 'Semantic Scholar'
  }
];

const COMPARE_DATA = [
  {name:'Midjourney',emoji:'🎨',bg:'linear-gradient(135deg,#7c3aed,#4f46e5)',free:'✕',price:'$10/mo',quality:'★★★★★',speed:'★★★★☆',api:'✕',commercial:'✓',rating:'4.9',best:true},
  {name:'DALL-E 3',emoji:'🖼️',bg:'linear-gradient(135deg,#10b981,#059669)',free:'✓ limited',price:'$0.04/img',quality:'★★★★☆',speed:'★★★★★',api:'✓',commercial:'✓',rating:'4.5',best:false},
  {name:'Adobe Firefly',emoji:'✨',bg:'linear-gradient(135deg,#ff6b2b,#d97706)',free:'✓ 25/mo',price:'CC sub',quality:'★★★★☆',speed:'★★★★☆',api:'~',commercial:'✓✓',rating:'4.6',best:false},
  {name:'Leonardo AI',emoji:'🎭',bg:'linear-gradient(135deg,#8b5cf6,#4f46e5)',free:'✓ 150/day',price:'$12/mo',quality:'★★★★☆',speed:'★★★★★',api:'✓',commercial:'✓',rating:'4.6',best:false},
  {name:'Bing Creator',emoji:'🔷',bg:'linear-gradient(135deg,#0ea5e9,#2563eb)',free:'✓ unlimited',price:'Free',quality:'★★★☆☆',speed:'★★★☆☆',api:'✕',commercial:'✓',rating:'3.8',best:false},
];

const WRITING_COMPARE = [
  {name:'ChatGPT',emoji:'🤖',bg:'linear-gradient(135deg,#10b981,#059669)',free:'✓',price:'$20/mo',quality:'★★★★★',ctx:'128k',api:'✓',int:'100+',rating:'4.8',best:true},
  {name:'Claude 3.5',emoji:'🧠',bg:'linear-gradient(135deg,#f59e0b,#d97706)',free:'✓',price:'$20/mo',quality:'★★★★★',ctx:'200k',api:'✓',int:'50+',rating:'4.8',best:false},
  {name:'Gemini Pro',emoji:'💎',bg:'linear-gradient(135deg,#4fa3ff,#a78bfa)',free:'✓',price:'$20/mo',quality:'★★★★☆',ctx:'1M',api:'✓',int:'Google WS',rating:'4.5',best:false},
  {name:'Jasper AI',emoji:'✍️',bg:'linear-gradient(135deg,#f43f5e,#e11d48)',free:'✕',price:'$49/mo',quality:'★★★★☆',ctx:'Custom',api:'✓',int:'Surfer SEO',rating:'4.5',best:false},
  {name:'Copy.ai',emoji:'📋',bg:'linear-gradient(135deg,#8b5cf6,#7c3aed)',free:'✓',price:'$36/mo',quality:'★★★☆☆',ctx:'Standard',api:'✓',int:'Zapier',rating:'4.1',best:false},
];

const VIDEO_COMPARE = [
  {name:'Runway Gen-3',emoji:'🎬',bg:'linear-gradient(135deg,#f43f5e,#be123c)',free:'✓ 525cr',price:'$15/mo',quality:'★★★★★',len:'10s',res:'720p/1080p',rating:'4.7',best:true},
  {name:'Kling AI 2.0',emoji:'🎥',bg:'linear-gradient(135deg,#7c3aed,#4f46e5)',free:'✓ 66cr/day',price:'$8/mo',quality:'★★★★★',len:'10s',res:'1080p',rating:'4.7',best:false},
  {name:'Luma Dream',emoji:'🌈',bg:'linear-gradient(135deg,#06b6d4,#0891b2)',free:'✓ 30/mo',price:'$29.99/mo',quality:'★★★★☆',len:'5s',res:'720p',rating:'4.4',best:false},
  {name:'Pika Labs',emoji:'⚡',bg:'linear-gradient(135deg,#f59e0b,#d97706)',free:'✓',price:'$8/mo',quality:'★★★☆☆',len:'4s',res:'720p',rating:'4.0',best:false},
  {name:'Sora',emoji:'🚀',bg:'linear-gradient(135deg,#10b981,#059669)',free:'✕',price:'$20/mo',quality:'★★★★★',len:'60s',res:'1080p',rating:'4.9',best:false},
];

const TRENDING = [
  {num:'01',name:'Sora',cat:'Video Generation',up:'+340%',color:'#f43f5e'},
  {num:'02',name:'Claude 3.5 Sonnet',cat:'AI Assistant',up:'+210%',color:'#f59e0b'},
  {num:'03',name:'Midjourney v7',cat:'Image Generation',up:'+180%',color:'#7c3aed'},
  {num:'04',name:'Cursor AI',cat:'Code Editor',up:'+165%',color:'#06b6d4'},
  {num:'05',name:'Gamma App',cat:'AI Presentations',up:'+140%',color:'#10b981'},
  {num:'06',name:'Perplexity Pro',cat:'AI Research',up:'+125%',color:'#0ea5e9'},
  {num:'07',name:'HeyGen 3.0',cat:'AI Avatars',up:'+118%',color:'#f472b6'},
  {num:'08',name:'Kling 2.0',cat:'Video Generation',up:'+95%',color:'#fb923c'},
];

const ARTICLES = [
  {
    slug: 'best-ai-video-generator-2026',
    emoji: '🎬',
    thumb: 'linear-gradient(135deg,#0a0020,#1a0050,#0d0030)',
    cat: 'ROUNDUP',
    date: 'May 1, 2026',
    title: 'Best AI Video Generator Tools in 2026 — Free & Paid Compared',
    excerpt: 'AI video generation has exploded. We tested Sora, RunwayML, Kling AI, Pika, Synthesia, InVideo AI and Luma Dream Machine so you know exactly which one to use — and which to skip.',
    keywords: 'best AI video generator 2026, AI video generator free, text to video AI',
    readTime: '9 MIN READ',
    author: 'AI Prime Hub Editorial',
    authorRole: 'Tools Research Team',
    authorEmoji: '🎬',
    tags: [
      'AI Video',
      '2026',
      'Free Tools',
      'Comparison',
      'Text to Video'
    ],
    fullContent: `
      <p class="article-lead">If you searched for the best AI video generator in 2026, you are in exactly the right place. We spent three weeks testing every major text-to-video AI tool — running identical prompts, measuring output quality, checking free plan limits, and calculating real-world value. This is the most complete, hands-on comparison you will find anywhere.</p>
      <div class="article-toc">
        <div class="article-toc-title">📋 What's in this guide</div>
        <ul>
          <li><a href="#what-is">What is an AI video generator?</a></li>
          <li><a href="#why-2026">Why 2026 is the tipping point</a></li>
          <li><a href="#top7">Top 7 AI video tools — ranked</a></li>
          <li><a href="#comparison">Full comparison table</a></li>
          <li><a href="#verdict">Final verdict</a></li>
        </ul>
      </div>
      <h2 id="what-is">What Is an AI Video Generator?</h2>
      <p>An AI video generator is a tool that turns your text prompts, images, or scripts into fully produced video clips — automatically, without any camera, editing software, or prior experience. You describe what you want, and the AI renders it.</p>
      <p>These tools use large video diffusion models trained on millions of hours of footage to understand motion, lighting, camera angles, and visual storytelling. The best ones in 2026 produce results that are genuinely hard to distinguish from real footage.</p>
      <div class="article-callout"><p>💡 <strong>Quick fact:</strong> The global AI video market is projected to exceed $2.5 trillion in spending by end of 2026 — making this the fastest-growing segment in all of AI.</p></div>
      <h2 id="why-2026">Why 2026 Is the Tipping Point for AI Video</h2>
      <p>A year ago, most AI-generated videos looked obviously artificial — shaky hands, warped faces, stuttering motion. That era is over. The 2026 generation of video AI tools produces output that rivals professional production at a fraction of the cost and time.</p>
      <p>The driving force is a combination of improved video diffusion architectures, larger training datasets, and massive compute investments from OpenAI, Google, and a wave of well-funded startups. What used to cost $5,000 in studio time now costs $15 a month.</p>
      <h2 id="top7">Top 7 Best AI Video Generator Tools in 2026</h2>
      <h3>🥇 1. Sora by OpenAI — Best Overall Quality</h3>
      <p><strong>Free plan:</strong> Limited | <strong>Paid:</strong> From $20/month</p>
      <p>Sora is OpenAI's flagship text-to-video model and it is, without question, the most cinematic AI video generator available in 2026. Feed it a detailed prompt and it delivers up to 60-second videos with realistic lighting, coherent motion, and an almost uncanny grasp of physical space.</p>
      <p><strong>Best for:</strong> Filmmakers, creative agencies, high-end marketing content. <strong>Rating: ⭐⭐⭐⭐⭐</strong></p>
      <h3>🥈 2. RunwayML Gen-3 Alpha — Best for Creators</h3>
      <p><strong>Free plan:</strong> Yes (525 credits) | <strong>Paid:</strong> From $15/month</p>
      <p>RunwayML has been the professional creator's choice for two years running. What separates Runway from the pack is not just video generation — it is the entire suite of AI video editing tools wrapped around it. <strong>Rating: ⭐⭐⭐⭐⭐</strong></p>
      <h3>🥉 3. Kling AI 2.0 — Best Free Option in 2026</h3>
      <p><strong>Free plan:</strong> Yes — 66 credits/day | <strong>Paid:</strong> From $8/month</p>
      <p>Kling AI from Kuaishou is the biggest surprise of 2026. Kling 2.0 produces video quality that rivals tools costing three times as much, and its free plan is genuinely usable. <strong>Rating: ⭐⭐⭐⭐½</strong></p>
      <h3>4. Pika Labs 2.0 — Best for Social Media</h3>
      <p>Pika is purpose-built for short, punchy social content. The interface is clean and beginner-friendly, and results come back fast. <strong>Rating: ⭐⭐⭐⭐</strong></p>
      <h3>5. Synthesia — Best for Business Presentations</h3>
      <p>Synthesia creates professional talking-head videos using AI avatars. For HR training videos, product explainers, and e-learning content, Synthesia is unbeatable. <strong>Rating: ⭐⭐⭐⭐</strong></p>
      <h3>6. InVideo AI — Best for YouTube Channels</h3>
      <p>InVideo AI is the closest thing to a one-click YouTube channel. You describe your video idea and InVideo writes the script, selects B-roll, adds voiceover, and exports a complete video. <strong>Rating: ⭐⭐⭐⭐</strong></p>
      <h3>7. Luma Dream Machine — Best for Product Videos</h3>
      <p>Luma Dream Machine specialises in smooth, fluid motion. Upload a product image and a text prompt, and Luma will generate a gorgeous dynamic product video. <strong>Rating: ⭐⭐⭐½</strong></p>
    `
  },
  {
    slug: 'midjourney-vs-dalle-vs-firefly',
    emoji: '🏆',
    thumb: 'linear-gradient(135deg,#0d0521,#2d0a6b)',
    cat: 'COMPARISON',
    date: 'Jan 28, 2025',
    title: 'Midjourney vs DALL-E 3 vs Firefly: The 2025 Image AI Showdown',
    excerpt: 'We tested all three on 50 identical prompts. The results were closer than expected, but one winner is clear.',
    tags: [
      'Image AI',
      'Midjourney',
      'DALL-E',
      'Firefly',
      'Comparison'
    ],
    readTime: '7 MIN READ',
    author: 'AI Prime Hub Editorial',
    authorRole: 'Image AI Specialist',
    authorEmoji: '🎨',
    fullContent: `<p class="article-lead">We ran 50 identical prompts through Midjourney, DALL-E 3, and Adobe Firefly to find out which image AI is actually best in 2025. The results surprised us.</p><h2>The Test Setup</h2><p>We tested each tool on portrait photography, abstract art, product photography, typography, and architectural visualization. Each prompt was run 3 times and the best result was selected for fair comparison.</p><h2>Winner: Midjourney (Still)</h2><p>Midjourney remains the gold standard for artistic quality and creative range. Its v6 model produces stunning, painterly results that neither DALL-E 3 nor Firefly can match for sheer beauty.</p><h2>Runner-up: Adobe Firefly (for commercial use)</h2><p>If you need commercially safe images, Firefly wins hands down. Every image is trained on licensed content — zero legal risk. The quality has also improved dramatically.</p><h2>Best free option: DALL-E 3 via Bing</h2><p>DALL-E 3 is available completely free via Bing Image Creator. For most everyday needs, it delivers impressive results at no cost.</p>`
  },
  {
    slug: '10-free-ai-tools-replace-expensive-software',
    emoji: '💡',
    thumb: 'linear-gradient(135deg,#001a10,#003d1f)',
    cat: 'GUIDE',
    date: 'Jan 25, 2025',
    title: '10 Free AI Tools That Replace Expensive Software',
    excerpt: 'From Photoshop to Premiere Pro — these free AI tools can do 80% of the job at zero cost.',
    tags: [
      'Free Tools',
      'Budget',
      'Alternatives'
    ],
    readTime: '6 MIN READ',
    author: 'AI Prime Hub Editorial',
    authorRole: 'Tools Research Team',
    authorEmoji: '💡',
    fullContent: `<p class="article-lead">You do not need to spend thousands on software subscriptions. These 10 free AI tools can replace some of the most expensive software in the world — and in many cases, they're better.</p><h2>1. Remove.bg — Replace Photoshop Background Removal</h2><p>Background removal in Photoshop used to take 20 minutes. Remove.bg does it in 5 seconds, completely free, with better edge detection than most professionals can achieve manually.</p><h2>2. CapCut — Replace Premiere Pro for Social Media</h2><p>For short-form video editing, CapCut is free and faster than Premiere for the tasks that matter on social media — auto captions, background removal, trending templates.</p><h2>3. Canva AI — Replace Adobe Express</h2><p>Canva's free tier is genuinely useful with AI generation, templates, and brand tools. The paid tier ($15/mo) is a fraction of the Adobe CC cost.</p><h2>4. Grammarly — Replace ProWritingAid</h2><p>The free version of Grammarly catches 90% of errors. Upgrade only if you need the advanced style suggestions.</p><h2>5. NotebookLM — Replace Expensive Research Tools</h2><p>Google's NotebookLM is completely free and lets you upload 50 sources and chat with your research. No research subscription needed.</p>`
  },
  {
    slug: 'cursor-ai-deep-dive',
    emoji: '🚀',
    thumb: 'linear-gradient(135deg,#150500,#3d1200)',
    cat: 'REVIEW',
    date: 'Jan 22, 2025',
    title: 'Cursor AI Deep Dive: Is It Really the Future of Coding?',
    excerpt: 'We used Cursor for 30 days as our primary IDE. Here\'s what we found — including the frustrating parts.',
    tags: [
      'Cursor',
      'Code AI',
      'IDE',
      'Review'
    ],
    readTime: '8 MIN READ',
    author: 'AI Prime Hub Editorial',
    authorRole: 'Developer Tools Specialist',
    authorEmoji: '💻',
    fullContent: `<p class="article-lead">We switched our entire development workflow to Cursor AI for 30 days. Here's our honest, detailed review — the good, the great, and the genuinely frustrating parts.</p><h2>What Cursor Actually Is</h2><p>Cursor is a VS Code fork with AI deeply integrated throughout. It is not a plugin — it is a completely separate editor that is API-compatible with VS Code extensions.</p><h2>The Tab Autocomplete Is Extraordinary</h2><p>Cursor's Tab key predicts not just the next line, but entire blocks of code. After two days of use, you start to feel like you're navigating a code graph rather than writing characters.</p><h2>Composer Mode Changes Everything</h2><p>Composer mode lets you describe a change and Cursor makes it across multiple files simultaneously. "Add dark mode to my React app" actually works.</p><h2>The Frustrating Parts</h2><p>Context limits are real. On large codebases, Cursor sometimes loses track of architecture decisions made in other files. The $20/mo plan can feel restrictive on heavy usage days.</p><h2>Verdict: Yes, it's the future</h2><p>After 30 days, not one member of our team wants to go back to vanilla VS Code. Cursor is the most significant productivity upgrade we've found in years.</p>`
  },
  {
    slug: 'ai-prompt-engineering-101',
    emoji: '🎨',
    thumb: 'linear-gradient(135deg,#050015,#15003d)',
    cat: 'GUIDE',
    date: 'Jan 7, 2025',
    title: 'AI Prompt Engineering 101: Get Better Results Every Time',
    excerpt: 'Master the art of prompting to unlock the full potential of image and text AI tools.',
    tags: [
      'Prompt Engineering',
      'Guide',
      'AI Tips'
    ],
    readTime: '5 MIN READ',
    author: 'AI Prime Hub Editorial',
    authorRole: 'AI Researcher',
    authorEmoji: '🎨',
    fullContent: `<p class="article-lead">The difference between a mediocre AI output and an extraordinary one is usually the prompt. Prompt engineering is the skill of communicating clearly with AI — and it can be learned in minutes.</p><h2>The 5-Part Prompt Formula</h2><p>The best prompts include: (1) Context — who you are and why you need this, (2) Task — what specifically you want, (3) Format — how you want the output structured, (4) Tone — the voice and style, (5) Constraints — what to avoid.</p><h2>For Image Generation</h2><p>Be specific about style: "photorealistic, 8K, studio lighting, shallow depth of field" is better than "good photo." Reference artists whose work matches your vision: "in the style of Ansel Adams" or "anime style by Studio Ghibli."</p><h2>For Writing AI</h2><p>Give the AI a role: "You are a senior copywriter with 15 years of experience writing for luxury brands." Then give it your task. Role assignment dramatically improves output quality.</p><h2>Chain Your Prompts</h2><p>Complex tasks work better in steps. First generate an outline, then expand each section, then refine the tone. One mega-prompt rarely beats a thoughtful chain.</p>`
  }
];

// ─── Category tool meta (for generated tools) ───
const CATEGORY_TOOL_META = {
  'Image Generation':{keyword:'Image',tags:['text-to-image','styles','upscaling','prompting'],features:['Generate images from text prompts','Style presets and reference modes','Upscaling and variation tools','Batch exports for production use','Fast cloud rendering workflow'],alts:['Leonardo AI','Playground AI','Bing Image Creator'],freeAlt:'Leonardo AI'},
  'Video Editing':{keyword:'Video',tags:['editing','transitions','captions','timeline'],features:['AI-powered timeline editing','Auto captions and translations','Scene cleanup and reframing','Template-based export presets','Fast social-ready rendering'],alts:['Descript','Kapwing','Pika Labs'],freeAlt:'Kapwing'},
  'Writing & Copy':{keyword:'Writing',tags:['copywriting','blogs','ads','editing'],features:['Draft blogs, ads and landing copy','Tone and brand voice controls','Rewrite and summarize content','SEO-friendly content suggestions','Team review and export flows'],alts:['Copy.ai','Writesonic','Gemini'],freeAlt:'Copy.ai'},
  'Code Assistant':{keyword:'Code',tags:['autocomplete','debugging','review','refactor'],features:['Inline code suggestions and chat','Bug detection and explanations','Refactor and test assistance','Project-aware code generation','IDE and repo workflow support'],alts:['Codeium','Cursor','Tabnine'],freeAlt:'Codeium'},
  'PDF & Docs':{keyword:'Docs',tags:['pdf','summaries','ocr','extract'],features:['Summarize PDFs and long docs','Extract tables and key fields','OCR for scanned documents','Answer questions from files','Structured export to notes or CSV'],alts:['Humata','ChatPDF','AskYourPDF'],freeAlt:'ChatPDF'},
  'Research':{keyword:'Research',tags:['citations','web search','analysis','sources'],features:['Search the web with citations','Organize findings into briefs','Compare sources side by side','Extract facts from large datasets','Share research collections with teams'],alts:['You.com','Consensus','Phind'],freeAlt:'You.com'},
  'Notes & PKM':{keyword:'Notes',tags:['notes','pkm','workspace','summaries'],features:['Capture and organize ideas quickly','Summarize meetings and notes','Auto-link related knowledge','Search across personal docs','Turn notes into action items'],alts:['Obsidian','Mem.ai','Bear'],freeAlt:'Obsidian'},
  'Audio & Music':{keyword:'Audio',tags:['music','beats','vocals','sound'],features:['Generate music from prompts','Create stems and loops quickly','Voice and instrument enhancement','Royalty-friendly exports','Fast remix and extend tools'],alts:['Udio','Stable Audio','Beatoven'],freeAlt:'Udio'},
  'Voice & Speech':{keyword:'Voice',tags:['tts','voice cloning','speech','dubbing'],features:['Natural text-to-speech output','Voice cloning and dubbing tools','Multilingual narration workflows','Audio cleanup and mastering','API-ready speech generation'],alts:['PlayHT','Murf AI','Kokoro TTS'],freeAlt:'PlayHT'},
  'Data Analysis':{keyword:'Data',tags:['analysis','charts','csv','insights'],features:['Analyze spreadsheets with AI','Generate charts and dashboards','Explain trends and outliers','Create summaries from raw data','Export insights for reports'],alts:['Rows AI','Akkio','Julius AI'],freeAlt:'Julius AI'},
  'Graphic Design':{keyword:'Design',tags:['templates','branding','layouts','creative'],features:['Generate layouts and brand assets','Smart resize for every format','Template-based campaign creation','Background cleanup and compositing','Export packs for teams or clients'],alts:['Adobe Express','Microsoft Designer','VistaCreate'],freeAlt:'Microsoft Designer'},
  'Chatbots':{keyword:'Chatbot',tags:['chatbot','support','knowledge base','agents'],features:['Build website and support bots','Train on FAQs and docs','Escalate conversations to humans','Embed on sites and apps quickly','Track replies and conversation quality'],alts:['Botpress','Tidio AI','Manychat'],freeAlt:'Botpress'},
  'SEO & Marketing':{keyword:'Marketing',tags:['seo','keywords','campaigns','traffic'],features:['Keyword clustering and content briefs','SEO scoring and optimization','Ad and campaign copy generation','Competitor tracking dashboards','Performance suggestions for growth'],alts:['Surfer SEO','NeuronWriter','Semrush AI'],freeAlt:'NeuronWriter'},
  'Email & Outreach':{keyword:'Outreach',tags:['email','prospecting','sequencing','personalization'],features:['Write personalized cold emails','Generate follow-up sequences','Score leads and prospects quickly','Track opens and reply patterns','Sync with CRM and outreach tools'],alts:['Instantly','Smartlead','Mailmeteor'],freeAlt:'Mailmeteor'},
  'Automation':{keyword:'Automation',tags:['workflows','agents','zaps','integrations'],features:['Connect apps with AI workflows','Trigger automations from prompts','Map fields and transform data','Monitor runs and failure states','Deploy reusable team automations'],alts:['Zapier','Make','n8n'],freeAlt:'n8n'},
  'Photo Editing':{keyword:'Photo',tags:['retouch','background','enhance','portrait'],features:['Retouch portraits and products','Remove backgrounds instantly','Enhance low-light or blurry images','Apply batch edits with presets','Export social and print-ready files'],alts:['Lensa AI','Pixlr','PhotoRoom'],freeAlt:'Pixlr'},
  'AI Assistants':{keyword:'Assistant',tags:['assistant','reasoning','tasks','productivity'],features:['Chat, reason and summarize quickly','Handle daily task workflows','Work across files and web context','Create drafts, plans and answers','Support personal and team productivity'],alts:['Claude','Gemini','Pi AI'],freeAlt:'Gemini'},
  'Productivity':{keyword:'Productivity',tags:['meetings','notes','automation','workflow'],features:['AI meeting transcription & summaries','Action item extraction','Calendar and task automation','Cross-app workflow integration','Team collaboration & async tools'],alts:['Otter AI','Fathom','Fireflies'],freeAlt:'Fathom'},
  '3D & AR':{keyword:'3D',tags:['3D model','AR','animation','game assets'],features:['Text-to-3D model generation','Real-time 3D scene creation','AR-ready asset export','Physics simulation','Browser-based 3D design'],alts:['Spline AI','Meshy AI','Luma AI'],freeAlt:'Spline AI'},
};

const GENERATED_TOOL_PREFIXES=['Nova','Atlas','Nimbus','Pixel','Echo','Lumen','Orbit','Vector','Flux','Spark','Quantum','Craft'];
const GENERATED_TOOL_SUFFIXES=['Studio','Flow','Pilot','Suite','Forge','Cloud','Desk','Lab','Works','Pro'];
const GENERATED_TOOL_BACKGROUNDS=['linear-gradient(135deg,#7c3aed,#4f46e5)','linear-gradient(135deg,#10b981,#059669)','linear-gradient(135deg,#f43f5e,#be123c)','linear-gradient(135deg,#06b6d4,#0891b2)','linear-gradient(135deg,#f59e0b,#d97706)','linear-gradient(135deg,#0ea5e9,#0284c7)','linear-gradient(135deg,#a855f7,#7c3aed)'];

let nextGeneratedToolId=Math.max(...TOOLS.map(t=>t.id),0)+1;

function buildGeneratedTool(cat,index){
  const meta=CATEGORY_TOOL_META[cat.name]||{keyword:cat.name.split(' ')[0],tags:['ai','workflow','productivity','automation'],features:['Fast AI workflow support','Smart suggestions and assistance','Simple setup and exports','Team-friendly collaboration','Reliable cloud-based tools'],alts:['ChatGPT','Gemini','Claude'],freeAlt:'Gemini'};
  const pricing=['free','freemium','paid'][index%3];
  const price=pricing==='free'?'Free':pricing==='freemium'?`Free / $${8+(index%6)*2}/mo`:`From $${12+(index%8)*4}/mo`;
  const rating=Number((4.1+(index%8)*0.1).toFixed(1));
  const reviews=`${(1.2+(index%14)*0.5).toFixed(1)}k`;
  const name=`${GENERATED_TOOL_PREFIXES[index%GENERATED_TOOL_PREFIXES.length]} ${meta.keyword} ${GENERATED_TOOL_SUFFIXES[Math.floor(index/GENERATED_TOOL_PREFIXES.length)%GENERATED_TOOL_SUFFIXES.length]} ${Math.floor(index/(GENERATED_TOOL_PREFIXES.length*GENERATED_TOOL_SUFFIXES.length))+1}`;
  return{id:nextGeneratedToolId++,name,cat:cat.name,emoji:cat.icon,bg:GENERATED_TOOL_BACKGROUNDS[index%GENERATED_TOOL_BACKGROUNDS.length],pricing,badge:index%17===0?'best':(index%9===0?'new':''),url:`https://www.google.com/search?q=${encodeURIComponent(name+' '+cat.name+' AI tool')}`,desc:`${name} helps teams handle ${cat.name.toLowerCase()} workflows with ${meta.tags.slice(0,2).join(', ')} and faster production-ready output.`,rating,reviews,tags:meta.tags,features:meta.features,alts:meta.alts.map((altName,ai)=>({n:altName,t:ai===0?'Free':(ai===1?'Freemium':'Paid')})),price,freeAlt:meta.freeAlt};
}

function ensureCategoryTools(cat){
  const currentCount=TOOLS.filter(t=>t.cat===cat.name).length;
  if(currentCount>=cat.count)return;
  for(let i=currentCount;i<cat.count;i++){
    const tool=buildGeneratedTool(cat,i);
    TOOLS.push(tool);
  }
}
